...
export function generateCompatCSS(): string {
  ...
  [data-theme="light"] {
    ...
    /* NOTE: do NOT alias --radius back to var(--radius-md) here. The
       tokens define --radius: 2px and --radius-sm/md/lg/xl: var(--radius),
       so pointing --radius at --radius-md forms a custom-property CYCLE.
       CSS makes cyclic properties invalid at computed-value time, which
       silently emptied --radius, --radius-sm/md/lg/xl and every
       --ds-radius-* alias — so every rounded-sm/md/lg/xl utility in the
       whole design system fell back to 0px. --radius is already correct
       from starui-tokens.css; nothing to alias. */
    ...
    /* Aliases for token names that drifted in component code — defined
       here so every referencing surface resolves (audit found these
       consumed but never defined). */
    --ds-border-default:   oklch(var(--border));
    --ds-text-tertiary:    oklch(var(--muted-foreground));
    --ds-accent-primary:   oklch(var(--primary));
    --fx-font-sans:        var(--font-sans);
    ...
    /* AI assistant identity accent — see --bot-accent in starui-tokens.css.
       Scoped to the chat assistant UI only; not a general-purpose accent. */
    --ds-bot-accent:            oklch(var(--bot-accent));
    --ds-bot-accent-foreground: oklch(var(--bot-accent-foreground));
    --ds-bot-accent-deep:       color-mix(in oklch, oklch(var(--bot-accent)) 80%, oklch(var(--foreground)) 20%);
    --ds-bot-accent-soft:       oklch(var(--bot-accent) / 0.10);
    --ds-bot-accent-border:     oklch(var(--bot-accent) / 0.38);
    --ds-bot-accent-ring:       oklch(var(--bot-accent) / 0.30);
