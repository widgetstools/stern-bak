 ...
 * exist when those modules are registered. A reduced module list registers none of
...
import { generalSettingsModule } from '../customizer/modules/general-settings/index.js';
import { gridStateModule } from '../customizer/modules/grid-state/index.js';

/** Reduced list omitting the expression-consuming modules. */
const REDUCED_MODULES = [generalSettingsModule, gridStateModule];
import { useSsrmExpressionBridge } from './useSsrmExpressionBridge.js';

function renderWithModules(modules: typeof DEFAULT_MODULES) {
...
describe('useSsrmExpressionBridge — module presets', () => {
  it('mounts under REDUCED_MODULES, which registers none of the expression modules', () => {
    expect(() => renderWithModules(REDUCED_MODULES)).not.toThrow();
  });
