/**
 * BlotterHost — stale-data banner wiring from provider status events
 * (disconnect → banner + dataStale; loading/ready → clear). Moved from
...
const providerConfig = { providerType: 'mock', keyColumn: 'id', columnDefinitions: [{ field: 'id' }, { field: 'price' }] };
const providerRow = { configId: PROVIDER_ID, name: 'Stale Test Provider', config: providerConfig };

type MockProvider = IDataProvider & {
...
function createMockProvider(): MockProvider {
  ...
  return {
    ...
    onSnapshotData: vi.fn((handler: (rows: unknown[]) => void): Unsubscribe => { snapshotHandlers.add(handler); return () => snapshotHandlers.delete(handler); }),
    ...
    emitSnapshot(rows: unknown[]) { for (const handler of snapshotHandlers) handler(rows); },
  };
}

let latestProvider: MockProvider | null = null;
  ...
  MarketsGrid: (props: any) => {
    ...
    React.useEffect(() => {
      ...
      const t = setTimeout(() => {
        ...
        props.onReady({ gridApi: api, platform: {}, profiles: {}, saveAll: vi.fn() });
      }, 0);
      return () => clearTimeout(t);
    }, [props.onReady]);
    return <div data-testid="markets-grid-stub" data-stale={props.dataStale ? 'true' : 'false'} data-stale-message={props.dataStaleMessage ?? ''} />;
  },
...
vi.mock('@wellsfargo-starui/react/data/runtime', () => ({
  ...
  useDataProvider: (id: string | null | undefined) => {
    if (id !== PROVIDER_ID) {
      latestProvider = null;
      return { provider: null, status: 'loading' as ProviderStatus, error: undefined, start: vi.fn(), refresh: vi.fn(), restart: restartMock };
    }
    latestProvider ??= createMockProvider();
    return { provider: latestProvider, status: 'loading' as ProviderStatus, error: undefined, start: vi.fn(), refresh: vi.fn(), restart: restartMock };
  },
  ...
  useDataProviderConfig: (id: string | null | undefined) => ({ cfg: id === PROVIDER_ID ? providerRow : null, loading: false }),
...
const baseProps = {
  componentName: 'Test',
  gridId: 'g-stale',
  defaultInstanceId: 'inst-stale',
  defaultAppId: 'app-1',
  defaultUserId: 'u1',
  configManager: fakeConfigManager,
} as const;

describe('BlotterHost — provider stale state', () => {
  beforeEach(() => {
    latestProvider = null;
  ...
  it('sets dataStale on provider error and clears after loading→ready', async () => {
    const adapter = makeAdapter({ liveProviderId: PROVIDER_ID, historicalProviderId: null, mode: 'live' });
    render(<BlotterHost {...baseProps} storage={vi.fn(() => adapter) as any} onError={noopOnError} />);
    ...
    await waitFor(() => { expect(lastMarketsGridProps.current?.dataStale).toBe(false); });
  ...
  it('auto-refreshes grid data after disconnect then ready', async () => {
    ...
    latestProvider!.refresh = vi.fn().mockImplementation(async () => { latestProvider!.emitSnapshot(refreshedRows); });
