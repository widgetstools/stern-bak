/**
 * BlotterHost — primary toolbar date → historical provider mode, restart
 * payload, historical-view banner, and save-and-switch. Moved from
...
const liveProviderRow = { configId: LIVE_PROVIDER_ID, name: 'Live Provider', config: { providerType: 'mock', keyColumn: 'id', columnDefinitions: [{ field: 'id' }, { field: 'price' }] } };
const histProviderRow = { configId: HIST_PROVIDER_ID, name: 'Historical Provider', config: { providerType: 'mock', keyColumn: 'id', columnDefinitions: [{ field: 'id' }, { field: 'price' }] } };

function createMockProvider(id: string): IDataProvider & { start: ReturnType<typeof vi.fn>; emitStatus: (status: ProviderStatus, error?: string) => void } {
  const statusHandlers = new Set<(status: ProviderStatus, error?: string) => void>();
  return {
    id,
    capabilities: { providerType: 'mock', streaming: true, realtime: true, supportsRefresh: true, supportsRestart: true },
    ...
    onRowsReceived: vi.fn(() => () => undefined), onSnapshotData: vi.fn(() => () => undefined), onTick: vi.fn(() => () => undefined), onError: vi.fn(() => () => undefined),
    onStatus: vi.fn((handler: (status: ProviderStatus, error?: string) => void): Unsubscribe => { statusHandlers.add(handler); return () => statusHandlers.delete(handler); }),
    emitStatus(status: ProviderStatus, error?: string) { for (const handler of statusHandlers) handler(status, error); },
  };
}
...
vi.mock('@wellsfargo-starui/grid', () => ({
  MarketsGrid: (props: any) => {
    ...
    React.useEffect(() => {
      ...
      const api = { setGridOption: vi.fn(), getDisplayedRowCount: () => 0, flushAsyncTransactions: vi.fn(), applyTransactionAsync: vi.fn(), getRowNode: () => null };
      const t = setTimeout(() => {
        if (readySentRef.current) return;
        readySentRef.current = true;
        props.onReady({ gridApi: api, platform: {}, profiles: {}, saveAll: saveAllMock });
      }, 0);
      return () => clearTimeout(t);
    }, [props.onReady]);
    return <div data-testid="markets-grid-stub" data-toolbar-date={props.toolbarDate ?? ''} data-historical-view={props.historicalViewMode ? 'true' : 'false'} />;
  },
...
vi.mock('@wellsfargo-starui/react/data/runtime', () => ({
  ...
  useDataProvider: (id: string | null | undefined) => {
    if (!id) return { provider: null, status: 'loading' as ProviderStatus, error: undefined, start: vi.fn(), refresh: vi.fn(), restart: restartMock };
    if (!providers.has(id)) providers.set(id, createMockProvider(id));
    return { provider: providers.get(id)!, status: 'loading' as ProviderStatus, error: undefined, start: vi.fn(), refresh: vi.fn(), restart: restartMock };
  },
  ...
  useDataProviderConfig: (id: string | null | undefined) => ({
    cfg: id === LIVE_PROVIDER_ID ? liveProviderRow : id === HIST_PROVIDER_ID ? histProviderRow : null,
    loading: false,
  }),
  useResolvedCfg: (cfg: unknown) => cfg,
  useDataProvidersList: () => ({ configs: [liveProviderRow, histProviderRow] }),
  useAppDataStore: () => ({ store: { get: vi.fn(), set: appDataSet, list: () => [], subscribe: vi.fn() } }),
}));
...
const baseProps = {
  componentName: 'Test',
  gridId: 'g-hist',
  defaultInstanceId: 'inst-hist',
  defaultAppId: 'app-1',
  defaultUserId: 'u1',
  configManager: fakeConfigManager,
} as const;

describe('BlotterHost — toolbar historical mode', () => {
  beforeEach(() => {
    providers.clear();
  ...
  it('switches to historical provider and restarts when a past toolbar date is selected', async () => {
    const adapter = makeAdapter({ liveProviderId: LIVE_PROVIDER_ID, historicalProviderId: HIST_PROVIDER_ID, mode: 'live' });
    render(
      <BlotterHost
        {...baseProps}
        storage={vi.fn(() => adapter) as any}
    ...
    await waitFor(() => {
      ...
      expect(restartMock.mock.calls.some(([extra]) => extra?.asOfDate === '2026-04-01')).toBe(true);
    }, { timeout: 10_000 });
