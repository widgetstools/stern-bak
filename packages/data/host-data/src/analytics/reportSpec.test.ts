 ...
 * for was dropped by the very validation every spec passes, and the window
 * stamped "whenever it happened to open" instead.
 */
describe('the as-of time', () => {
  const withAsOf = (asOf: unknown) => ok(report({ asOf } as never)).asOf;

  it('keeps the time the model said the data is for', () => {
    expect(withAsOf('2026-09-01T14:32:00Z')).toBe('2026-09-01T14:32:00Z');
  });

  it('survives the round trip through storage', () => {
    const once = ok(report({ asOf: '2026-09-01T14:32:00Z' } as never));
    expect(ok(JSON.parse(JSON.stringify(once))).asOf).toBe('2026-09-01T14:32:00Z');
  });
