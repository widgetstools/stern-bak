/**
 * engineHost — a readable, multi-session host for the Perspective engine
 * WASM, derived from the library's own `perspective-server.worker.js`
 * shim (whose engine-host classes are bundled but not exported, and
 * whose single module-level session breaks multi-client hosting).
 * Validated in `apps/source/perspective-spike` (Phase 0 hosting gate);
 * vendored here for the `dataPlane: 'engine'` provider sub-worker.
 *
...
function makeImports(getExports: () => EngineExports | null, heap: () => Uint8Array): WebAssembly.Imports {
  ...
  const env: Record<string, unknown> = {
    ...
    proc_exit(code: number) {
      // eslint-disable-next-line no-console
      console.error('[psp-engine] proc_exit', code);
      return 0;
    },
