...
export function createDataServicesClient(
  ...
  // `:ssrm2` forces a fresh SharedWorker when the hub protocol gains features
  // (e.g. stomp-ssrm). Browsers pin the first script for a given `name` until
  // every tab closes — without a bump, rebuilds leave a stale hub running.
  const worker = new SharedWorker(new URL('../../assets/data-services-worker.mjs', import.meta.url), {
    type: 'module',
    name: `mkt-data-services:${opts.appName}:ssrm2`,
  });
