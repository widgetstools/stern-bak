...
| `cdp-hidden-liveness.mjs` | Two facts per view: is it alive while hidden (100 ms interval fired n / 80 over 8 s, max gap, rAF frames — the earlier isolation revert's failure mode) and does the page KNOW it is hidden (`document.visibilityState`, `document.hidden`, and whether `fin` is present — OpenFin views expose no `isShowing()`; measured 2026-09-13: inactive docked tabs read `hidden`). The second is Phase C's entry criterion: the hub's `meta.hidden` comes from `document.hidden`. |
| `cdp-process-map.mjs` | On the OpenFin provider page (default `--url /platform/provider`): every child window's views with their renderer PID, working-set / private memory and CPU (`View.getProcessInfo`), `processAffinity` tag, plus a distinct-PID summary; the raw `fin.System.getAllProcessInfo()` tree goes to the JSON. Phase A acceptance (one PID per docked view, memory in budget). Exits 1 on a non-OpenFin page. |
...
| `cdp-cpu-profile.mjs` | Sampled CPU profile of a view over `--seconds` (DevTools `Profiler`, `--interval` µs): busy / idle / `(program)` / GC totals, inclusive time per `--match` pattern (time of samples whose stack contains a matching frame, counted once per sample — default patterns cover AG Grid's `executeBatchUpdateRowData` flush, the autosize `processResizeOperations` timer, `refreshCells`, and the data client's `handleMessage`), and the top self-time functions. The "flush + listener time per 10 s" number behind refactor plan B0 / B1. Sampling costs the page a little; run the lag probe separately. |
