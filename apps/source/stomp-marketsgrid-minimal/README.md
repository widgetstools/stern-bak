...
Same STOMP broker / `TRADER001` topics; the app seeds a `stomp-ssrm` catalog row and mounts `StarGrid`, which renders **`MarketsGrid` with the `ssrm` prop**. Expect the same host chrome as CSRM: primary toolbar, settings/customizer drawer, formatter toggle, edit toolbar toggle, and profile/save when `withStorage` is set. Hard-refresh after SharedWorker protocol bumps if the hub name is unchanged.

CSRM default (no query flag) is unchanged: `http://localhost:5213/` mounts `StarGrid` against the CSRM provider row.
...
| `src/App.tsx` | Idempotent catalog seed; `StarGrid` against the CSRM row, or the `stomp-ssrm` row when `?ssrm=1` |
...
- **[docs/latest/](../../../docs/latest/README.md)** — current documentation set (getting started, architecture, SSRM engine)
