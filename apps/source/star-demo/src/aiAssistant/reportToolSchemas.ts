        ...
        properties: {
          ...
          blocks: {
            ...
            description:
              ...
              '"chart" { query, chart?, style?, normalize?, scale?, baseline? } — chart is any of: ' + CHART_KINDS.join(', ') + '. For a stacked/grouped/multi-line chart give the query BOTH groupBy and pivotBy: the pivot values become the series. normalize:true makes each stack sum to 100% (share of total rather than absolute size). scale:"log" for measures spanning orders of magnitude, where the small values vanish against the large — impossible when the measure reaches zero or goes negative (P&L), and downgraded to linear with the reason in the caption. baseline:"auto" zooms the value axis to the data range instead of anchoring at zero, which is what makes tightly clustered values readable (prices between 98 and 103 are identical bars anchored at zero); on a bar chart it also exaggerates differences, so prefer it for lines and say what you did. '  +
