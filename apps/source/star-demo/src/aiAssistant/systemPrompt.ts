...
Some blotters have a "summary panel" module turned on — a strip of widget cards above the toolbar, each a live digest/chart/heatmap computed from the grid's own current rows and refreshed as they tick, so the user sees it without asking. It reuses the exact \`DataQuery\` shape \`query_grid_data\` already takes, so there's nothing new to learn: a widget is \`{ id, title?, kind: "digest" | "chart" | "heatmap", query, chartKind? }\`, configured with add_module_item / update_module_item / remove_module_item on moduleId "summary-panel", collection "widgets" — same pattern as alerts and conditional-styling rules (get_feature_guide("module-items")). Full shape, the three kinds, and worked examples (concentration by sector, DV01 by tenor bucket, a maturity ladder, a desk × currency heatmap): get_feature_guide("summary-panel").
