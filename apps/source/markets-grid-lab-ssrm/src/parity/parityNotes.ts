...
export const PARITY: ParityEntry[] = [
  ...
  },
  {
    ...
    label: 'Editing',
    status: 'partial',
    summary: 'Cell edits and paste persist through the engine write path; journal/undo does not.',
    notes: [
      ...
      'Edit history / undo-redo journal is CSRM-only (applyTransactionAsync is not a write path under SSRM).',
      'Smart Edit toolbar segments are disabled under SSRM (their patch path does not persist engine-side).',
    ],
  },
  {
    tabId: 'bulk-update',
    label: 'Bulk Update',
    status: 'gap',
    summary: 'Disabled under SSRM by the honesty locks.',
    notes: [
      'Bulk update writes through applyPatches/applyTransactionAsync, which the server-side row model ignores — it would look applied and silently not be.',
      'The toolbar disables with a status line instead of failing silently.',
      'Route: rebuild on ssrm-apply-edits (the write path pastes already use) — a scoped follow-up, not an engine gap.',
    ],
  },
  {
    tabId: 'plus-minus',
    label: 'Plus / Minus',
    status: 'gap',
    summary: 'Keyboard nudges no-op under SSRM.',
    notes: [
      'Same applyTransactionAsync write path as bulk update; disabled rather than silently non-persistent.',
      'Route: same ssrm-apply-edits rebuild as bulk update.',
    ],
  },
  {
    tabId: 'shortcuts',
    label: 'Shortcuts',
    status: 'gap',
    summary: 'Letter-key arithmetic no-ops under SSRM.',
    notes: [
      'Same write-path story as plus/minus; disabled with the honesty lock.',
      'Route: same ssrm-apply-edits rebuild.',
    ],
  },
  {
    tabId: 'profiles',
