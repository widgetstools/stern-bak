...
- SSRM wires CSRM's stale-data busy indicator: provider `error` after first `ready` sets `dataStale` on MarketsGrid (flashing `StaleDataBanner`, editing disabled), any recovery to `ready` clears it; cold-connect retries before first ready stay silent
