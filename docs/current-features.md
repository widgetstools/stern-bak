...
- `attachSsrmSession` / `getSsrmSession` / `isSsrmGrid` / `drainSsrmRows` / `exportSsrmVisualExcel` / `lockSsrmExpressionColumns` / `withSsrmSelectAll` / `sendSsrmClipboard` / `wrapSsrmContextMenu` — SSRM honesty locks. Excel/CSV drain the filtered book from the engine (refuse above `SSRM_EXPORT_MAX_ROWS`) instead of `exportDataAsExcel` on loaded blocks, and a selection export under header select-all excludes the `toggledNodes` the user un-ticked; calculated / expression columns (`context.staruiVirtual` / `staruiExpression`) cannot be sorted, filtered, or grouped; Advanced Filter is forced off (`toViewSpec` has no tree compiler); multi-row header select-all sets `selectAll: 'all'` so AG Grid 36 uses `setServerSideSelectionState`; clipboard still copies loaded text and says so. Smart Edit, Bulk Update, plus-minus, and letter-key shortcuts are disabled (edits do not persist to WASM / upstream). Alerts and conditional-styling header badges are labelled as loaded (visible) rows only
...
- `apps/source/stomp-ssrm-minimal` — `HostedSsrmMarketsGrid` against one catalog row; auto-starts `stomp-view-server` via `npm run app -- stomp-ssrm-minimal` (the launcher spawns through a shell and kills with `taskkill /T` on Windows). `?rate=<updates/sec>` on the demo URL sets the broker's live trigger rate for load runs (default 1000; the stored provider row is re-saved when the rate changes); the config declares no `searchColumns` (quick search covers every text column) and carries a `maturityDate` `dateString` column plus editable `trader` / `marketValue` / `pnl` / `currentPrice` for paste tests. `apps/scripts/ssrm-perf/` holds the Playwright measurement harnesses and WASM engine probes behind the SSRM baselines (README inside; run against `vite preview`, not the dev server). Same chrome as the CSRM demo: filters row, formatting toolbar (which also enables the View menu's Auto Format item), editing toolbar
...
- `ISsrmDataProvider` — SSRM client contract at `IDataProvider` lifecycle parity (`capabilities`, `start` / `stop` / `refresh` / `restart`, `getRows`, `getColumnValues`, `getRowCount`, `getAggregates`, `watchGroups`, `onSsrmTick`, `onRefresh`); no `getData()` / `onSnapshotData()` / `onTick()` — rows stay in the worker cache. `getAggregates` feeds the status bar and expression `SUM` / `AVG` / `MIN` / `MAX` / `COUNT`. Optional `applyEdits({ rows })` writes grid edits (whole rows) into the engine cache — every grid on the provider sees them and refreshes keep them; the upstream feed is not written to, so its next tick for that row wins. Optional `status` exposes the last hub status so the datasource can hold its first block until the snapshot is in
