...
- **STOMP** (`startStomp()`)
  ...
  - Data plane (`cfg.dataPlane: 'hub' | 'subworker'`, default `'hub'`; hub-wide default via `SharedWorkerDataServicesHubOpts.dataPlane`; editor switch "Dedicated transport worker"): `'subworker'` runs the provider's whole transport — socket, fast frame parser, conflation, projection **and chunk encoding** — in its own SharedWorker (`@wellsfargo-starui/data/assets/data-provider-worker.js`, name `starui-provider:<providerId>`, transports only) while the hub keeps cache / replay / fan-out. Chromium SharedWorkers cannot spawn workers, so the hub asks each attaching window (`provider-worker-needed` event) to construct / join the provider's worker and transfer its port (`provider-port` request, port in the transfer list; the window's connection keeps the worker alive, the first port drives the transport, later ones are spares); the hub drives it over `pw-start` (cfg + AppData snapshot + optional overlay) / `pw-restart` / `pw-stop` / `pw-appdata` (AppData mirror for `{{name.key}}` on reconnect) / `pw-ping`–`pw-pong` heartbeat (`providerWorkerProtocol.ts`, `providerWorkerEntry.ts`, `providerWorkerHost.ts`). Snapshot batches and live batches of ≥`LIVE_BIN_MIN_ROWS` rows are encoded on the worker with the hub's codec rule and transferred zero-copy (`pw-rows`); the hub decodes once for its cache and relays the same chunks to windows and the replay cache (`ProviderEmitEvent.rows.encoded`). Fail-soft: window without the asset / no port within `providerPortTimeoutMs` (4 s) / missed start ack or heartbeat → spare port → hub thread, recorded on the slot (`HubProviderIntrospectRow.dataPlane`); a deferred handle replays attach-time overlays once the port arrives. Windows enable it by passing `providerWorkerScriptUrl` (`?url` import of the asset) through `ensurePlatformReady` / `bootstrapDataServicesWithWorkerAsset` / `createAppDataServices` / `DataHubProvider`; `createDataServicesClient` resolves it inline. Measured with 2 STOMP providers at 20k rows/s each: hub thread 86% → 45%, provider workers 36–38% on their own cores (`apps/source/stomp-marketsgrid-minimal/scripts/subworkerBench.mjs`; see `docs/wasm-data-plane-plan.md` §6)
