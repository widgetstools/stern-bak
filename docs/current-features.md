...
- `useFilterModel` — filter-model persistence + mutation; per-pill counts route through `platform.data` (`filterPillCounts.ts`) in BOTH row models. A badge counts rows in the whole dataset matching that pill's own model (`scope: 'all'`) — neither the applied filter nor the quick-filter text narrows it, so two pills' badges stay comparable. Where `capabilities.canAddressUnloadedRows` holds (client-side row model), one `scan` pass builds counts and match sets together and streaming ticks patch them incrementally via `RowChangeBus` deltas; where it does not (server-side), one `count` per pill and no match sets, so every emit is a full recompute until roadmap Phase 5 lands a real server-side delta source. A pill whose model the shared predicate refuses over-counts and warns once rather than reading zero
