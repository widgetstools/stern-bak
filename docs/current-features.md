...
- **Calculated columns** — virtual cols from expressions; under SSRM the editor names the column's REAL tier from the engine expression contract classifier (`classifySsrmExpression`): `ENGINE-READY` (compiles to the wire grammar — unlocks with engine plan §12 T3, plus T4 for aggregate scalars / T6 for date parts), `GRID` (row-local but outside the grammar), `LOADED ROWS` (MEDIAN/STDEV/VARIANCE/DISTINCT_COUNT — their values are loaded-block statistics), with engine-total aggregates flagged; so nothing is silently blank
...
- `apps/source/markets-grid-lab-ssrm` — the Feature Lab's 17 tabs against the server-side row model (`:5301`): the lab's OWN `LabFeatureConfig`s (imported, so parity cannot drift) mounted through `MarketsGrid.ssrm` on one shared `mock-ssrm` provider — same worker generator as the CSRM lab, no broker. Home is a feature-by-feature parity matrix (full / partial / gap with the mechanism behind every gap, `src/parity/parityNotes.ts` — after plan §12 C1/C2 the write-path rows are full: 11 full / 5 partial / 0 gap); each tab repeats its verdict above the grid. Demo console pauses/paces the feed via `provider.restart` and injects the lab's scenarios through `ssrm-apply-edits` over the loaded blocks; client-synthetic valueGetter columns are branded `staruiSsrmClientExpr` so the honesty lock disables their sort/filter/group; the engine boot schema types every lab field with a drift-failing test
...
- `compileToEngineExpression` / `classifySsrmExpression` / `toComputedColumnSpec` — the SSRM engine expression contract (plan §12 T1): DSL AST → the versioned `SsrmExprNode` wire grammar (`@wellsfargo-starui/types` `ssrmExpression.ts` — operators, IN/BETWEEN, unified `cond`, scalar fn set, `agg` nodes for `SUM/AVG/COUNT/MIN/MAX([col])`), with anything outside v1 REPORTED untranslatable (variables, diff refs, REGEX_MATCH, NOW/TODAY/DATE_DIFF/DATE_ADD, loaded-row aggregates) — never guessed. Semantics are the CLIENT evaluator's, coercions and all, pinned by 45 golden fixtures (`ssrmExpressionContract.fixtures.json`) the Rust engine must reproduce at T3; the classifier's tiers (compiled / materialized / unsupported) + `requires` (aggregates→T4, dateFns→T6) feed the customizer chip
