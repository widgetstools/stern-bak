...
- `./perspective` — the Perspective row engine (see 3.2)
...
- **Row-engine picker (lab)** — every feature tab carries a `Client row model | Perspective (worker-held Table)` toggle, wired once into the shared `LabFeatureTab` shell through `TabContainer`'s `variants` slot. The two engines mount the SAME `gridId`, columns and profiles, so switching is an A/B of the row engine and nothing else. Each engine is its own component (`LabClientGrid` / `LabPerspectiveGrid`) so only one set of hooks is ever live, and switching remounts the grid — a row model is chosen when AG Grid is created, not changed underneath it. `usePerspectiveRows` is the Perspective twin of `useLabRows`: it attaches an inline `mock-perspective` cfg (which starts the worker slot the tee builds its Table off), ignores the rows that stream delivers, and only then attaches — the worker refuses `attachPerspective` for a provider that is not running, and that refusal reads as permanent. The demo console's tick slider and pause work identically on both, through the same `refresh(extra)`. **Scenarios report themselves unsupported** under Perspective rather than silently doing nothing (`LabStreamHandle.scenariosSupported`): every scenario patches rows through `gridApi.applyTransactionAsync`, a client-side row model's write path, and under Perspective the book is in the worker with nowhere for the patch to land. The lab boots the **Perspective** worker asset unconditionally, because which worker a window runs is settled at `new SharedWorker()` and the toggle has to work without a reload
- **Stress test tab** (`StressTestTab`, `buildStressColumns`) — the one tab that varies the book instead of the feature: 1,000–200,000 rows over ~120 columns, plus a "Second window" button. The other tabs run 500 rows over 20–40 columns, where both engines are comfortable and therefore indistinguishable; the multi-window story is only visible once there is enough data for the copies to matter and more than one window to hold them. The extra columns are **derived from real fields** (`Risk krd5Y · s2` reads `krd5Y`) with distinct `colId`s, so they sort, filter and aggregate rather than being inert padding an engine could shortcut
...
#### Row engine (`src/engine/`)

Which engine supplies a grid's rows. There are exactly two and there was never a third: the client-side model, where every window materializes the whole book, and Perspective, where the book lives once in a SharedWorker and each window reads only the blocks its viewport asks for.

- `MarketsGrid` props — `rowModel?: 'client' | 'perspective'` (absent = client, unchanged), `perspectiveTable`, `perspectiveKeyColumn`, `perspectiveQueries`, `perspectiveTreeFields`
- `GridSurfaceSlot` / `resolveGridSurface` — the ONE place a surface is chosen. Asking for `'perspective'` can never yield `'client'`: a missing table is `'pending'` (a sized, empty box), whether the attach is in flight or the host forgot to wire one. Structural, not defensive — a stand-in grid mounting during the async attach fires `onGridReady` (activating every module), then unmounts, and its `onGridPreDestroyed` calls `platform.destroy()` **permanently**; the real grid's later `onGridReady` then lands on a dead platform and every platform-driven feature (formatting toolbar, auto-formatter, saved-filter "+", profile save/restore) goes silently dead while AG's own features keep working
- `PerspectiveMarketsGridSurface` — the AG Grid surface in server-side row model, driven by `createPerspectiveRowEngine`; `makeGetRowId`
- `PerspectiveStatusPanel` — status-bar readout of the engine's state (row counts, view generation, refusals)
- `createPerspectiveEngineHolder` — a stable handle to a swappable engine. AG Grid reads `context` when it CREATES the grid and hands that exact value to every status panel; the engine is not stable (a provider restart hands over a new one, StrictMode double-invokes the mount effect), so the context carries the holder, whose identity never changes. `asPerspectiveContext` / `readPerspectiveContext` narrow it back off a grid api
- `createPerspectiveWorkerQueries` — the `PerspectiveGridQueries` seam (`watchCount`, `watchExpressionCount`, `watchAggregate`, `distinctValues`, `watchMatchSet`, `watchChangeRule`) over the worker's push protocol. Its four consumers — saved-filter count badges, the header painter's "does any row match", set-filter checkbox lists, and rule aggregates — used to answer by walking this window's own row nodes, which under Perspective answers about the viewport and presents it as the book. Nothing here polls; `null` is passed through untouched so a caller renders nothing rather than a plausible wrong number
- `withPerspectiveSetFilterValues` — routes a set filter's `filterParams.values` to the worker's distinct-values query
- `usePerspectiveCalcColumns` / `planCalcColumn` / `useCalcColumnsSnapshot` — three-tier plan for row-local calculated columns: compile to a Perspective expression → materialize into the row once on read → drop as unsupported. A client `valueGetter` cannot be sorted, filtered or grouped on server-side, so serving one that way is a silent correctness gap, not just a perf one
- `perspectiveAlertsBridge` (`customizer/modules/alerts/runtime/`) — per enabled rule, opens a `matchSet` / `changeRule` worker subscription and feeds `newlyMatched` rows into alerts' existing rule-tripped/history bookkeeping. `RowChangeBus` stays as-is and CSRM-only: Perspective alerts never route through it, because they have a correct signal instead of needing to impersonate a delta bus

### 3.2 `@wellsfargo-starui/grid/perspective`

**Path:** `packages/react-grid/perspective-grid`
**Purpose:** The window-side Perspective row engine. ONE Table lives in the SharedWorker; each blotter opens its own View and renders only the rows in its viewport, so a window never materializes the full book. **Nothing in this folder imports AG Grid** — its shapes are structural, so the engine unit-tests without a grid and without a DOM.

- `usePerspectiveTable(client, providerId, opts)` — provider id → the worker-held Table: `attachPerspective` for a port, build this window's Perspective `Client` on it, `open_table(name)`. The engine module is loaded dynamically so its wasm is fetched only by windows that actually open a blotter. **Refcounted per (client, providerId) with a deferred 1s teardown**, measured against React StrictMode: a naive per-effect attach ran setup → cleanup → setup, and the cleanup CLOSED the frame port while the Table handle opened over it kept being used — it read 0 rows forever while another client on the same worker read the full 20,000, with no error to explain it. Reports `status: 'idle' | 'attaching' | 'ready' | 'unavailable' | 'error'` with a `reason` — a refusal is a rendered sentence, never a spinner
- `createPerspectiveRowEngine()` — the row engine proper: block reads, sort/filter/group-expand, grand totals, cell edits, `readAllRows` / `readMatchingRows` one-shot pulls for Excel export. `GRAND_TOTAL_ROW_ID` / `GRAND_TOTAL_FLAG`
- `createPerspectiveDatasource()` — AG Grid's server-side datasource over a View. **Every `getRows` settles exactly once**: `maxConcurrentDatasourceRequests` (default 2) permanently wedges the grid on two leaked calls, so a stale generation or a missing view resolves empty rather than skipping the settle. Empty results **omit** `rowCount` rather than calling `setRowCount(n, true)`. `columnsToRows`, `cloneRequest`
- `createViewManager()` — View lifecycle keyed by config, with refcounts and update subscriptions (`PerspectiveTableLike`, `UpdatableView`)
- `createSafeView()` (re-exported from `@wellsfargo-starui/core`) — the ONLY way a View may be closed. Deleting one with a read in flight throws an **uncatchable** wasm borrow error that can take the whole SharedWorker down. It lives in core because the worker's query engine builds and drops Views under the same rule and cannot import from this bucket
- `createPerspectiveQueryClient()` — subscribe/unsubscribe over the control port with local dedup: two panels asking the same absolute question (`count` / `countExpression` / `aggregate` / `distinctValues`) share one worker subscription and the late one paints from the cached last result. `matchSet` / `changeRule` are never shared — those pushes say what CHANGED, so replaying one to a later subscriber would report transitions it was not around for
- `coerceEditedValue()` — cell-edit coercion against the column's Perspective type; `toPerspectiveEdits` / `GridDataTransaction` translate a grid transaction into Table edits (built and tested; the smart-edit / bulk-update / undo-redo write path is **not yet routed** to it — see the known gap in `PerspectiveMarketsGridSurface`'s header)
- `toGroupColumns`, `toTreeColumns`, `blankUnaggregatedNonNumeric`, `viewConfigKey`, `TREE_KEY_FIELD` / `TREE_GROUP_FIELD` — View-config helpers; the AG-request → View-config translation itself (`toPerspectiveViewConfig` and friends) lives in `@wellsfargo-starui/core` and is re-exported here so the grid layer has one import site
- `loadPerspectiveClient()` — the slim client build pointed at the wasm as a separate cacheable asset, rather than the inline build that also carries the server binary a window can never run
...
- `usePerspectiveGridBridge` — the container's end of the Perspective pull path, and the only place that knows both the selected provider and this window's data-services client. For a `*-perspective` provider it starts the provider (the worker refuses an attach for one that is not running, so the attach is held off behind `enabled` until `start()` resolves), attaches via `usePerspectiveTable`, builds the worker-query client, and hands `MarketsGrid` its `rowModel` / `perspectiveTable` / `perspectiveKeyColumn` / `perspectiveQueries`. For every other provider type it answers `rowModel: undefined` and the grid receives exactly the props it did before. Starting the provider is what this window does **instead of** wiring rows into the grid: `useProviderDataWiring` is handed a null provider on this path, so `applyProviderToGrid` — a client-side row model's write path — never runs. The loading overlay is driven by the attach rather than by a snapshot commit that never comes, and a refusal renders as `PerspectiveRefusalPanel` (the worker's own wording, over the pending surface so the provider picker stays reachable) rather than an open-ended spinner
...
- Creatable provider types (`SUPPORTED_TYPES`) — `stomp`, `stomp-perspective`, `rest`, `mock`, `mock-perspective`, `appdata`. A new draft is seeded from `getDefaultProviderConfig(type)` rather than `{ providerType }` alone, so a `mock-perspective` draft carries `rowShape: 'flat'` explicitly instead of leaning on a transport default the saved config would never record
- `ConnectionTab` — connection string, auth, transport selection; "Test Connection" button (STOMP/REST, and `stomp-perspective`, whose wire settings are identical) drives `useProviderProbe.test()`. STOMP runs a pure socket connect (`connectStomp` — handshake only, no subscribe/trigger/rows) and shows "Connected"; row-fetching transports (REST/mock) show "Connected — received N rows"
...
- `PerspectiveTableFields` — the five settings a `*-perspective` provider adds over its classic twin, owned once and composed by both wrappers rather than forked (both configs extend their twin by exactly the same five). `tableName` (defaults to the provider id — one Table per provider); the **declared schema** ("Declare from columns" writes `inferredFields`), which is the difference between a blotter that paints on open and one that waits out a whole snapshot, and whose current state is stated in words; `inferDates` (default on — off loses server-side date sort and range filtering); `integerColumns`, offered as a picker over only the numeric columns and captioned as the exception it is (Perspective silently **truncates** a float that lands in an integer column, one outlier row makes a sampled type wrong, and a float already represents every integer to 2^53 exactly); and `buildAfterRows`, the backstop for a feed with no snapshot end token. It also **checks** `keyColumn`, which is load-bearing on these types in a way it is not on their twins — anything but one scalar column makes the provider silently push-only — and surfaces `describePerspectiveKeyColumnRefusal`'s wording **before save**, so the editor says at authoring time what the worker would say at attach time
- `StompPerspectiveFields` — `StompFields` + `PerspectiveTableFields`. Composition, not a fork: `StompPerspectiveProviderConfig extends Omit<StompProviderConfig, 'providerType'>`, so every STOMP wire setting still applies verbatim and a change to either reaches both. `StompFields` / `MockFields` type their `cfg` as `Omit<…, 'providerType'>` for exactly this reason — neither reads the discriminant
- `MockPerspectiveFields` — `MockFields` + a row-shape control + `PerspectiveTableFields`. Its own component because of `rowShape`, which the STOMP twin has no equivalent of: the generated positions row is deeply nested and a Perspective schema is flat, so the rows must be flattened before they can reach a Table. Shown rather than left to the transport default, since a config that never records the setting is one that never decided it; warns when set to nested, and when flattening is on with no column definitions to lift paths from
...
- `./runtime/worker/perspectiveEntry` + `./assets/data-services-perspective-worker.mjs` — the second worker entry, identical to the default one (same handshake, same ConfigManager/catalog/AppData hydration, every provider type behaves the same) plus the `loadPerspective` hub option that `stomp-perspective` / `mock-perspective` build their Table on. A **separate asset** rather than a flag on the default one because `@perspective-dev/client/inline` embeds its wasm as base64: bundling it into the entry every app loads would cost every app megabytes even when it never opens a blotter
- **Which worker boots** — `perspective?: boolean` on `CreateDataServicesWorkerOpts`, `CreateDataServicesClientOpts`, `EnsurePlatformReadyOpts`, `EnsureHubOpts`, `WarmHubConnectionOpts` and `DataHubProvider`'s bootstrap form. Opt-in, for the size reason above; without it `client.attachPerspective(...)` answers `{ ok: false, reason: 'This SharedWorker hosts no Perspective engine…' }` and the pull path is unreachable. Both asset URLs are written out as their own inline `new URL(..., import.meta.url)` literal in `createDataServicesWorker` (the one place either is constructed — `createDataServicesClient` delegates to it) because a bundler only emits a worker whose URL it can see statically. An explicit `workerScriptUrl` still wins over the flag. `DATA_SERVICES_WORKER_ASSET` / `DATA_SERVICES_PERSPECTIVE_WORKER_ASSET` name the two package export paths
...
- **STOMP** (`startStomp()`)
  ...
  - `cfg.requestHeaders` — STOMP headers sent with the request (trigger) frame, verbatim. Without them a provider could only ask for whatever the broker did by default: `startStomp` published `{ destination, body }` only, so the in-repo fixture broker's own `snapshot-rows` (snapshot size) and `live-mode: sparse` (partial-row live deltas instead of full rows) were unreachable from an app. A broker's header vocabulary is its own, so nothing is whitelisted; the reserved frame headers (`destination`, `content-length`, `receipt`) are dropped rather than allowed to corrupt the frame (`sanitizeRequestHeaders`)
...
- **Mock** (`startMock()`)
  ...
  - `cfg.rowShape: 'nested' | 'flat'` (default `'nested'`) — `'flat'` lifts each `columnDefinitions[].field` path onto a literal top-level scalar key before emit, so `rating.moody` arrives as the key `"rating.moody"`. Named for the shape it produces rather than for any consumer: the generated positions row is deeply nested, and a consumer that cannot hold nested values — a Perspective Table's schema is a flat map of typed columns — needs it flat. With no `columnDefinitions` the flatten has nothing to lift and rows pass through unchanged (`flatRowShape.ts`)
  - `probeMock()` — one-shot probe
- **STOMP → Perspective** (`startStompPerspective()`) and **Mock → Perspective** (`startMockPerspective()`) — the same two transports with their emit stream **teed** into a Perspective Table hosted once in the worker, so a second and third blotter cost a View rather than a full replay. Thin composition over `startStomp` / `startMock` rather than forks: wire handling, reconnects, snapshot buffering and template resolution stay in one place. The classic push path is forwarded synchronously and unmodified, so anything already subscribed keeps working and the Table is a side effect rather than a replacement. Shared contract in `perspectiveTee.ts` (`PerspectiveTeeOpts`, `PerspectiveTeeHandle`: `feed` + `tableName`). A composite/array `keyColumn` **skips Table creation entirely** with an `index-invalid` diagnostic — Perspective indexes by one scalar column, and indexing on the first would let rows collide silently. Without a `perspectiveHost` (the default worker) the provider still runs, push-only

#### Worker-hosted Perspective Table

Only built on a worker booted from `perspectiveEntry` (see the `perspective` opt-in above); `host` and `queries` are non-null exactly together (`createPerspectiveSubsystem`).

- `createPerspectiveHost()` (`perspective/perspectiveHost.ts`) — owns the worker's engine: lazily loads it on first use, `tableFactoryFor(name)` hands a feed its table factory, `getTable(name)` / `onTableUpdate(name, cb)` serve the query engine, and `attach(port)` binds a window's ProxySession to a `MessageChannel` port. Deletion is **idempotent** — a hosted Table has two plausible owners (feed and host) and a second `delete()` throws fatally
- `createPerspectiveTableFeed()` (`perspective/perspectiveTableFeed.ts`) — turns a provider's emit stream into Table writes: `tap(emit)` wraps the emit callback, a declared schema builds the Table empty and immediately, otherwise rows buffer until the snapshot end token (or `buildAfterRows`). Emits `FeedDiagnostic`s (`index-invalid`, nested/mixed/unknown columns). Also keeps the **field-change shadow** (`changes`) — a per-field previous-value map, scoped to only the fields an active `changeRule` subscription watches, diffed **before** each `table.update()` lands, because an upsert Table retains no previous value
- `toPerspectiveSchema()` / `toPerspectiveSchemaFromFields()` (`perspective/perspectiveSchema.ts`) — schema from observed rows or from what the config already declares (`inferredFields` preferred over `columnDefinitions`: a field carries a real type where a column def carries a renderer hint). Every numeric column is `float` and `integer` is **opt-in** via `integerColumns` — `table.update()` coerces rather than rejects, so a mistyped integer column silently truncates, and a float represents every integer to 2^53 exactly. Nested columns are dropped; a declaration cannot distinguish date from datetime so date-ish columns map to `datetime`, which holds both
- `createPerspectiveQueryEngine()` (`perspective/perspectiveQueryEngine.ts`) — answers whole-book questions **once in the worker and pushes the results**, replacing the per-window rescan that is only correct while a window holds the whole book. The registry is keyed on the **translated View config**, so N windows watching one saved filter cost ONE refcounted View, not N. Re-evaluates on the Table's own `on_update`, throttled (`recomputeThrottleMs`). Serves all six `PerspectiveQuerySpec` kinds: `count`, `countExpression`, `aggregate`, `distinctValues` (refuses past `distinctValuesLimit` rather than truncating), `matchSet` (diffs a `Set<rowId>` and pushes only `{ newlyMatched, newlyUnmatched }`, refusing past `matchSetSnapshotCap`), and `changeRule` (served from the feed's shadow map, feeding the existing framework-agnostic `evaluateDataChangeRule` / `computeRelativeChange`). A question it cannot answer exactly comes back `{ kind: 'refused', reason }` on the same callback — a caller must render nothing rather than a plausible wrong number
- Protocol + RPC (`hubPerspectiveRpc.ts`, `protocol.ts`) — `perspective-attach` → `perspective-attach-result` (carrying a transferred `MessagePort` + the Table name), `perspective-query-subscribe` / `-unsubscribe` → pushed `perspective-query-result`. The attach port is a freshly minted `MessageChannel`, never the hub's own: Perspective's transport takes `onmessage` on whatever port it is given, so sharing the control port would make the two protocols overwrite each other's handler. `client.attachPerspective(providerId)` **never rejects** — no engine on this worker, a non-Perspective provider type, an unusable `keyColumn`, a provider that is not running, or a Table not built within `PERSPECTIVE_TABLE_WAIT_MS` all come back as `{ ok: false, reason }`, because a window that hears nothing cannot tell "still connecting" from "will never work". Query subscriptions are owned by the port, so a window disconnect drops every subscription it opened
- `describePerspectiveKeyColumnRefusal()` (in `@wellsfargo-starui/types/shared`) — the single wording for "this `keyColumn` cannot index a Perspective Table", shared by the attach RPC and the provider editor so authoring time and attach time cannot disagree
