...
- **StarUI v1 OKLCH tokens** (`tokens/starui-tokens.css`) — periwinkle accent (`#a3b1d2` dark / `#4f66a3` light), teal/rose buy/sell, mid-tone gray light (ground `#D5D6D7`, surface `#DEDEDE` — 10% darker than the original near-white ramp; body text `#2E2F31` at 9.2:1, muted `#4C4D4F` at 5.8:1, hairlines `#B4B6B8` and grid lines `#B6B7B9`) + blue-graphite dark; bare OKLCH components for alpha-friendly `oklch(var(--primary) / 0.12)` usage; `--bot-accent`/`--bot-accent-foreground` (indigo `#8093ff`) is a dedicated identity accent for the AI chat assistant UI, separate from `--primary`/`--accent` and `--chart-4` — since the accent moved to periwinkle the two now sit ~7° apart in hue and are told apart by chroma (vivid indigo vs muted periwinkle) rather than hue
...
- `createLiveRowSource()` / `LiveRowSource` / `LiveRowSourceProvider` / `useLiveRowSource` — the summary panel's data feed, fed by `IDataProvider.onSnapshotData` + `onTick` rather than the grid. Owns ONE array, mutates it in place on a tick, and reports change through a version counter, so a re-render that isn't about data (a dock drag, a resize) costs nothing. `MarketsGridContainer` builds it and supplies it through context; a consumer mounting `MarketsGrid` directly has no provider and falls back to the previous grid read
...
- Legend labels are built from the SPEC (`chartConfigFor`), not a fixed map. A combo of `pnl` bars against a `dailyPnl` line used to draw a legend reading "Value" and "Y" — naming neither measure — and a multi-series chart, whose dataKeys are its series keys, was absent from that map entirely and rendered swatches with no text
- Category axes share one tick config (`CATEGORY_TICKS`: `preserveStartEnd` + `minTickGap`) so labels thin themselves instead of overprinting. `interval={0}` forced every label to render whatever the width; the shared constant exists because the per-chart fix drifted apart once already
...
- `ReportSpec.asOf` is now carried through validation. It was declared on the type and
  ...
  fell back to stamping "whenever it happened to open".
