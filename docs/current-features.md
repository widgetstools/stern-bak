...
- `wireWorkerCatalogSync()` / `isCatalogConfigRow()` — `ensurePlatformReady` wires `ConfigManager.onConfigChanged` → `platformClient.invalidateConfig` only for `data-provider` / `appdata` rows (grid profile saves do not fan out `catalog-ready`); the platform-services worker is the only invalidation target — the data hub keeps no catalog
...
- two SharedWorker connections per window (worker-split) — `ensureDataServicesHub` owns a per-`appId` `HubConnection`: the platform-services worker (`mkt-platform-services:<appId>`, spawned FIRST, `platformClient` — catalog RPCs + AppData) and the data worker (`mkt-data-services:<appId>`, `client` — providers, CSRM deltas, SSRM RPCs); `warmHubConnection()` opens both early (never throws) so the workers spawn while ConfigManager init runs, and the hub bundle adopts the same ports (`bootstrapDataServices({ client, appDataClient })`); the bundle exposes both as `client` / `platformClient`, and `DataServices.platformClient` carries the second one to React
