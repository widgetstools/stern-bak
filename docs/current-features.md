...
- **STOMP** (`startStomp()`)
  ...
  - **Server-Side Row Model (Perspective replica)** — `MarketsGridContainer` `rowModel: 'serverSide'` mounts AG Grid's SSRM served by a **per-window Perspective (WASM) table** (`widgets-react/src/container/markets-grid-container/ssrm/`, exported via its OWN subpath `@wellsfargo-starui/grid/widgets/ssrm` — deliberately off the widgets barrel so consumers that never mount SSRM don't pay its import cost): `useSsrmData` replaces the CSRM grid-transaction wiring (same provider subscription, same status/overlay/start-kick semantics — `useProviderDataWiring` gains an `enabled` opt-out) with a replica table (`createSsrmFeedTable`: schema from ColDefs via `buildSchemaFromColDefs`/`typeForColDef`, snapshot = clear + columnar flattened chunks, ticks CONFLATE by row id into one pending map (last write wins — bounded by book size) with a single in-flight engine drain of ≤5k rows (**backpressure**: one engine write per tick batch let a slow engine grow an unbounded write queue → renderer OOM after minutes at 20k rows/s), flattened at drain time through the COMPILED plan (`compileFlattenPlan` trie, ~3.6µs/row vs ~20µs generic walk — the difference between the feed costing ~45% and ~7% of the main thread, which is what let held-arrow-key navigation starve) and written COLUMNAR by default (~10× cheaper against an indexed table; `sparseTicks` — plumbed from `cfg.thinDeltas` — keeps row-oriented writes so sparse deltas cannot null unmentioned fields; a snapshot supersedes ticks still waiting to drain), live-patch rows materialised LAZILY per visible row (update events carry ids only; drained values are stored FLAT — retaining raw 64-field nested rows for a whole ticked book cost several times the schema's flat projection), block cache deliberately small (`maxBlocksInCache` 20 by default — the "server" is a local engine so a block re-fetch is a millisecond view read, while caching the whole book kept every row resident as AG nodes and defeated the windowed model's memory point; raise it for remote datasources), index `__pspIndex` = `composeRowId` over `keyColumn` — leaf row ids byte-for-byte the CSRM ids) and a datasource (`PerspectiveSsrmDatasource`) answering every block/group/aggregate/filter/pivot/grand-total request from cached live engine views (`ViewCache` LRU, flat-rollup group levels with engine child counts, typed group routes via `resolveGroupRoute` so null/numeric/date group keys drill correctly, AG filter models translated to native Perspective filters + derived-column expressions, engine-illegal aggregates dropped-and-reported, null-proof `max` rewrite); live updates patch rendered leaf rows from the feed, patch on-screen group aggregates/grand total from the engine, and reload on top-level count changes — all behind a **scroll-quiescence gate** (the flush defers while the viewport scrolled within the last 200ms and catches the whole backlog up at rest, so patching never fights a fling for frame budget). Independently of every gate and mode (`liveUpdates: 'off'` included), cached engine views are **drained on data arrival** (`ViewCache.pollAll` — a cheap `num_rows` per view, time-gated 1/s): in a hidden tab Chromium throttles timers toward 1/min while ports keep streaming, and un-consumed live views accumulate engine-side state against a streaming table until the renderer OOMs — arrival is the one signal a backgrounded tab still receives at full rate. Grid surface gains `serverSideGridOptions` (spread last at mount; `rowData` withheld) threaded MarketsGrid → Host → Surface; wasm binaries resolve from the provider-worker asset URL (`SharedWorkerDataServicesClient.providerWorkerAssetUrl` + `engineAssetsFromWorkerUrl`), falling back to CSRM when absent. Demo: stomp-marketsgrid-minimal `?ssrm`
