...
- `npm run bench:ssrm` (`scripts/bench-ssrm.mjs`, `ROWS`/`COLS`/`BLOCK` env overrides) — measures ingest, cold/warm blocks, a 20-block scroll and tick cost against the built runtime, so these characteristics can't silently regress
