...
- `DataProviderEditor` — connection + tabs (Connections, Fields, Columns, Diagnostics). Supported transports include `iab-channel-bridge` (upstream provider picker + optional channel name; schema copied from upstream for grid key/columns). Sidebar **Import** button creates a brand-new persisted provider from an exported JSON config (`configStore.save` mints a fresh `providerId`, owned by the current user — or `system` when the config is public), then selects and opens it for editing; footer **Export** button downloads the current working config — including unsaved edits — as JSON. **Clone** (sidebar row or form footer) deep-clones the provider config into an unsaved draft that appears immediately in the sidebar list (tagged **Unsaved**) until the user saves — then `configStore.save` mints a real `providerId` and the row becomes persisted
...
- DataProvider type contracts (`PROVIDER_TYPES.STOMP_SSRM` / `StompSsrmProviderConfig` / `blockSize` / `publishWindowMs` / `searchColumns`; `PROVIDER_TYPES.IAB_CHANNEL_BRIDGE` / `IabChannelBridgeProviderConfig` / `DEFAULT_MARKETSUI_DATA_HUB_CHANNEL`)
- FieldSelector types
- Configuration types
...
- `ChannelBridgeClientAdapter` — `IDataProvider` for `iab-channel-bridge` catalog rows; attach/detach/refresh/restart over OpenFin IAB Channel (`CHANNEL_HUB_ACTIONS`, default channel `marketsui-data-hub` via `@wellsfargo-starui/data/channel-hub`)
...
- `useDataProvider(providerId, opts?)` — hub-backed `IDataProvider` wrapper (`ProviderClientAdapter` or `ChannelBridgeClientAdapter` when `opts.providerType === 'iab-channel-bridge'`); preferred over `useProviderStream` for production grids
...
- `./channel-data-hub` — `installChannelDataHub` (dock-hosted SharedWorker → IAB fan-out for cross-origin grids)
