...
- `MarketsGrid` `ssrm` prop (`{ provider, keyColumn?, cacheBlockSize? }`) — switches the inner surface to `rowModelType: 'serverSide'` while the host chrome (customizer, toolbars, profiles, settings) stays CSRM-identical. The AG Grid instance is created **once per provider** (BOTH mount sites — `MarketsGridHost` and `MarketsGridCore` — key the surface on `ssrm:${provider.id}` only; `MarketsGridCore` used to append the key column, which remounted the whole grid the moment `keyColumn` resolved from its `'id'` fallback and defeated the late-bound design below, T3-14): `keyColumn` is late-bound — it resolves after provider ready, `getRowId` is a stable function reading a ref (only the function is init-only), and a keyColumn change rebinds the datasource + tick binding in place with a full block purge instead of remounting the grid (no throwaway instance, one license-manager validation per load; React StrictMode still double-mounts in dev). Safe because no row can exist under the stale key: rows only load once the provider is ready, which is also what triggers the resolve and the purge. When `ssrm` is set, `rowData` is ignored. `MarketsGridSsrmProps` is on the public barrel (`grid/src/index.ts` `export type` block) — this line used to claim it was not
- `MarketsGridSsrmSurface` — SSRM inner surface; composes datasource, tick binding and status bar. **Internal** — mounted by `MarketsGrid`, not on the public barrel. It passes AG Grid **no grid-instance `modules` list**: instance modules are additive to the global registry, so the `[AllEnterpriseModule]` it used to pass handed every server-side grid the full enterprise bundle and made a reduced `agGridModules` prop silently inert. `MarketsGridSurface` never had one either — `ensureAgGridModules(agGridModules)`, called once by the MarketsGrid shell, is the single source for both row models. The surface's own no-arg `ensureAgGridModules()` is only the fallback for a standalone mount (tests, bare embeds); registration latches after the first call, so under MarketsGrid it is a no-op
- **The SSRM loading placeholder is a per-cell row, not a full-width one.** `MarketsGridSsrmSurface` sets `suppressServerSideFullWidthLoadingRow`, and `ssrmDefaultColDef` carries the same `BlankLoadingCellRenderer` (renders nothing — a blank row rather than AG Grid's default spinner + "Loading...", which flickered under a fast scrollbar-thumb drag) at the colDef level, since AG Grid only honours the grid-option-level renderer while a loading row is full-width. A full-width loading row has its own keyboard-navigation codepath (`processFullWidthRowKeyDown`) whose `ensureIndexVisible` → `redraw` re-mounts row components synchronously inside AG Grid's `flushSync` wrapper; holding ArrowDown/ArrowUp (key-repeat) against a streaming SSRM grid could remount enough rows within one flush to trip React's nested-update guard ("Maximum update depth exceeded"), crashing the grid. Suppressing the full-width row removes that codepath entirely
- **Fast SSRM scrolling debounces block loads.** `MarketsGridSsrmSurface` sets `blockLoadDebounceMillis={100}` — AG Grid's own documented lever for "scrolling over many blocks" — so held-arrow-key repeat, scrollbar drag and wheel scrolling wait for scrolling to settle before requesting/redrawing each intermediate block, rather than firing one request/redraw per row crossed. A custom `navigateToNextCell` override that coalesced key-repeat into batched jumps (applied once per animation frame via `setFocusedCell`/`ensureIndexVisible`) was tried and reverted: it fought AG Grid's own keyboard/focus/selection/scroll-pinning machinery — all normally driven together by the native per-keydown path — trading the main-thread jank it targeted for a jumpy focus ring and intermittent hangs, worse than the native behaviour it replaced. Native per-keydown navigation is what runs today
- **Live SSRM ticks share the same MAX UPDATES/SEC cap CSRM already has, not a second knob.** `bindSsrmTicks`'s unsorted "patch visible leaves in place" path used to call `applyServerSideTransaction` synchronously on every qualifying live tick, unthrottled — a real main-thread cost under a continuously-ticking feed (the same `flushSync`-driven React reconciliation the general-settings module already measured at "20k rows × 10k updates/sec → 60-170ms per flush" for the CSRM side, via `asyncTransactionWaitMillis`, which has no effect on the server-side row model's `applyServerSideTransaction`). `bindSsrmTicks` now accepts `updateThrottleMs`, and `MarketsGridSsrmSurface` reads back the SAME computed `asyncTransactionWaitMillis` value general-settings already derives from MAX UPDATES/SEC — one user-facing control governing both row models. Ticks landing inside the window coalesce into one flush covering the union of changed rows (latest value wins per row key); a window where any tick carried no `columns` (whole-row flash) never narrows to a partial column list. `0` (uncapped) preserves the original synchronous-per-tick behaviour exactly. Alert-hit reporting stays on the raw per-tick cadence — deliberately not batched, since it's a distinct latency-sensitive signal. The coalescing itself (`createSsrmRowPatcher`, `bindSsrmTicks.ts`) is hoisted to module scope capturing nothing, matching this repo's standard remediation for adding to an already-over-budget function (`docs/WORKLOG.md` item 21)
- **Held-arrow-key repeat is rate-limited at the DOM level, ahead of AG Grid's own handler — not replayed.** `useThrottledArrowKeyRepeat` (`packages/react-grid/grid/src/widget/`, wired into `MarketsGridSsrmSurface` via the surface root ref) attaches a `capture`-phase `keydown`/`keyup` listener above AG Grid's own row-container listener. The first 200ms of a hold (and every single, non-repeat tap) passes through completely untouched; once `KeyboardEvent.repeat` has been true for longer than that, excess repeats within each ~80ms window are dropped (`stopPropagation` + `preventDefault`) so AG Grid never sees them, converting native OS auto-repeat (~30-60/sec) into evenly-spaced discrete presses — the shape a fast manual tapper produces, not a continuous hold. Every event that DOES get through is handled by AG Grid's native `navigateToNextCell`/focus/selection/scroll-pinning machinery completely unmodified, which is the point: an earlier `navigateToNextCell` override that replayed navigation itself (accumulating deltas, driving `setFocusedCell`/`ensureIndexVisible` manually) bypassed that machinery for the throttled case and produced a jumpy focus ring and intermittent hangs; this doesn't touch navigation logic at all, only which raw keydowns are allowed to reach it. Resets on `keyup`, so a fresh hold is unthrottled again from zero. Skips interception entirely when the event target is a text-editing element (`<input>`/`<textarea>`/`contenteditable`) so in-cell editing's own cursor movement is never affected. Verified via CDP against a live OpenFin session: a 30s continuous raw-repeat hold crossing many SSRM block boundaries showed zero blank rows and zero scroll-stall streaks throughout
...
- `bindSsrmTicks()` — routes provider tick events to `applyServerSideTransaction` (`BindSsrmTicksOptions`); reports each transaction's changed nodes to `platform.rows` (`BindSsrmTicksOptions.rows`, wired by `MarketsGridSsrmSurface`), which is the server-side row model's ONLY row-change delta source — `applyServerSideTransaction` fires no `asyncTransactionsFlushed`; also subscribes provider status and **purges all blocks on every transition into `ready`** — a broker restart/reconnect or a provider restart/refresh re-snapshots upstream, and since status fans to every session, every subscribed grid repopulates automatically with no page reload (the snapshot flush alone fires on the first streamed chunk and later chunks are interest-gated, so ready is the only complete-cache signal)
- **An SSRM grid says what it is doing.** `useSsrmProviderStatus` turns the provider's RAW status stream into the four things the chrome needs (the wiring hook's `onStatus` receives display text, so it cannot tell `ready` from an error whose message reads like a status; its identity must also stay stable or the wiring effect restarts the provider every render). The **loading overlay** (`MarketsGridLoadingOverlay`, the same component CSRM uses) covers the grid from mount until the first load SETTLES, naming the provider and counting streamed rows; a post-ready re-snapshot brings it back with "Refreshing" copy, and a profile write shows "Saving…" (the host's own `onSavingChange` is chained, not replaced). The first load settles on **ready or error** — the overlay takes pointer events across the whole grid including the toolbar, so leaving it up on a failed provider would remove the only route to repairing that provider. `ready` from the wiring hook is a second settle signal, because the status subscription is optional-chained and a transport need not have one. Then the **stale banner** owns everything after: any current provider error sets `dataStale` (flashing `StaleDataBanner`, editing disabled) and any recovery to `ready` clears it. The banner's COPY splits on whether the feed ever delivered — "Live SSRM feed disconnected — …, values may be stale" once it has, and "Cannot load data from this provider — …. Pick or repair it in Custom Settings → DATA PROVIDER." when it never did, because staleness is a meaningless claim about data that never arrived. An error before the first ready used to be suppressed entirely, which left a permanently-failing provider showing an empty grid and no explanation
- **A grid with no provider still mounts.** Where the resolved provider id is empty there is no adapter to bind, and the container mounts MarketsGrid on the `__none__` sentinel row key with empty columns, the data-infra admin pair (no refresh/reload — there is nothing to refresh) and a banner naming the problem, so the toolbar, the customizer's DATA PROVIDER card and the Data Provider Editor all stay reachable. CSRM's `__no_provider__` shell, same shape. While grid-level data is still being read the container renders CSRM's small centred placeholder rather than a grid it would remount when the persisted selection lands
- **Reconnect resync needs nothing extra** — `bindSsrmTicks` already purges every block on each transition INTO `ready` (`bindSsrmTicks.statusRefresh.test.ts` pins both the recovery purge and the no-double-purge case), so a broker restart or a provider restart/refresh re-queries rather than keeping pre-disconnect values. The roadmap listed this as an open gap; it was closed before the roadmap was written
...
- **Filter-pill badges count the whole cache** — `computeFilterPillCounts` routes every pill through `platform.data.count({ scope: 'all', filterModel })`, which the server-side adapter answers with a zero-row block (`startRow: 0, endRow: 0` → `rowCount` with nothing materialised) against the whole worker row store, not the loaded blocks a `forEachNode` scan would see. The quick filter is deliberately NOT folded in: the badge means the same thing in both row models, and that meaning is the client-side one (roadmap binding constraint 1). Applied pill filters and the quick filter still evaluate worker-side for the GRID's rows, via `setFilterModel` / SSRM query state. Streaming ticks patch the badges from the row-change delta rather than recounting — see `useFilterModel` above
- **`SsrmMarketsGridContainer` accepts the host surface `MarketsGridContainer` accepts.** It `extends Omit<MarketsGridProps, …>` and SPREADS the rest onto the grid, where it previously listed 26 props by hand and dropped **29** members of the 55-member `MarketsGridProps` — `modules`, `agGridModules`, `sideBar`, `statusBar`, `rowHeight`, `headerHeight`, `animateRows`, `className`, `componentName`, `tabsHidden`, `toolbarActionsLayout`, `onGridReady`, `onSavingChange`, `showVisualExcelExport`, `sizeColumnsToFitOnReady`, `includeAllStreamSafeFilters`, `autoSaveDebounceMs`, `storageAdapter`, the historical/toolbar-date group, and the caption group among them. Same change makes `StarGrid`'s `advanced` escape hatch live for an SSRM grid: StarGrid spreads `advanced` onto the container, and the container now spreads it onward. **Mode-specific members, all of them held back deliberately:** `ssrm` (the container owns it — that is what makes it the SSRM container), `rowData` (ignored by the grid whenever `ssrm` is set), `rowIdField` / `columnDefs` (facts about the provider, resolved by `useSsrmColumnResolution` — CSRM omits both for the same reason), `gridLevelData` / `onGridLevelDataLoad` (the container owns grid-level persistence, as in CSRM), and `headerExtras` (absent from the CSRM container's surface too). `gridId` is optional here where CSRM requires it, because `providerId` is a required unique provider key and makes a sensible default. Three further deliberate divergences: the container defaults `showFiltersToolbar` / `showFormattingToolbar` / `showEditingToolbar` **on** where MarketsGrid's own defaults are off/deferred (a host value always wins; removing them would lower SSRM, which roadmap constraint 1 forbids), and `userId` defaults to `LOGGED_IN_USER_ID` because the storage adapter is keyed from it
- Host `style` **merges over** the container's `{ height: '100%', width: '100%' }` fill rather than replacing it — same precedence MarketsGrid's own root style uses for its `style` prop. That fill is what gives the AG Grid viewport a real height (`apps/e2e/star-demo-ssrm-smoke.spec.ts` asserts > 200 px, the collapse the full-bleed hosted frame exists to prevent), so a rest spread that let a host value replace it would re-open that bug. `className` and `style` now mean what they mean in `MarketsGridProps` — the grid root — where the container previously applied both to its own wrapper div, which is also what `StarGrid`'s `advanced` type had always claimed they were
- `SsrmMarketsGridContainer` container-specific props — `providerId` (required; the default live provider), `defaultHistoricalProviderId`, `inlineCfg`, `expressionSnapshot`, `showProviderEditor` / `showStatusStrip` (both default **false** so hosted layouts match star-demo exactly; the inline editor dialog stays reachable from the admin action with the strip hidden), `onEditProvider` / `onOpenConfigBrowser` (OpenFin popout seams), `onError`, `gridEventHandlers` / `handlerMeta`, `historicalDateAppDataRef`, `onRowIdFieldChange` (resolved key column), `onProviderReady` (live `ISsrmDataProvider`). The container-specific diff against `MarketsGridContainerProps`, in full: SSRM-only are the six provider-model props just listed (`providerId`, `inlineCfg`, `expressionSnapshot`, `showProviderEditor`, `showStatusStrip`, `onProviderReady`) — a worker-plane provider has an inline transport cfg, an expression snapshot, its own chrome strips and a live handle a colour-link host resolves selections through; CSRM-only is now just `defaultLiveProviderId`, whose SSRM equivalent is the required `providerId`. `onRowIdFieldChange` reports `string | null` here against CSRM's `string | readonly string[] | null`, because `resolveSsrmKeyColumn` collapses a composite key to the `__ssrmRowId` sentinel before it reaches the grid
- **Provider editor and Config Browser route on the RUNTIME, not on callback presence.** Both containers now read `isOpenFin()`: under OpenFin the host callback opens its popout, in a browser the container opens the shadcn dialog. The SSRM container used to route on `onEditProvider` being supplied and to add the `config-browser` action only when `onOpenConfigBrowser` was wired — so a browser host that supplied its OpenFin callbacks (every hosted app does) could reach neither surface. Its data-infra menu is byte-for-byte `MarketsGridContainer`'s and both entries are unconditional: `refresh-view`, `reload-from-source`, `data-provider-editor`, `config-browser` — same ids, labels, icons, order, with host `adminActions` merged last and winning on id collision (`mergeAdminActions`, shared by both containers)
- `onError` reaches the SSRM container — `useSsrmProviderDataWiring` had accepted and wired it since it was written, but no container prop fed it, so stream errors only ever reached `statusText` and were invisible whenever the status strip was off (its default). A failed `Reload from source` reports through it too. Held in a ref and exposed as a stable callback, because the wiring effect keys on `onError` identity and would restart the provider on every render otherwise — the same hazard `onStatus` already carried
- **A caption edit on an SSRM grid survives a remount.** The toolbar renders `EditableCaption` unconditionally, and the SSRM container used to pass a static `title` with no `onCaptionChange` — so the user could rename the grid and lose it on the next mount, and an OpenFin "Save Tab As…" rename never reached the toolbar. Both containers now share `useContainerCaption`: the caption persisted in the grid-level row wins over the `caption` prop, a commit writes back and chains the host's handler, and a genuine POST-mount change to the prop is adopted under OpenFin (the initial value never is, so a caption stored before tab-name binding existed survives until the tab is actually renamed). `useGridLevelPersistence` had been loading and saving the caption for SSRM grids all along — only the two consumers were missing. The container's `title` prop is gone; `caption` is the one name, as in CSRM (`StarGrid` passes `tabTitle || title || gridId` to both branches)
- **The grid-event subsystem works on an SSRM grid.** `useContainerEventWiring`, shared by both containers, owns `useMarketsGridEventBridge`, the Custom Settings `GridEventBindingsHostApi` (catalog, handler ids, `handlerMeta`, both setters), and the `provider:switched` / `provider:dataStale` emits; the container creates the bus and emits `provider:status` from the stream it owns — for SSRM that is the provider's raw `onStatus`, the same subscription that drives the stale banner. So an app passing `gridEventHandlers` can bind grid, profile and provider events on either row model, and the bindings persist in the same grid-level row. `toolbar:dateChanged` is the one catalog event an SSRM grid still never emits: the container has no toolbar-date handler of its own until the historical subsystem lands
- **SSRM cell-editor `valuesSource` bindings resolve.** The container supplies `appData` from `useAppDataLookup` (shared with CSRM), where it previously never imported the store at all, so every `{{providerName.key}}` binding in column-customization resolved to nothing on a server-side grid
- **Historical mode round-trips on a server-side grid.** It used to be half-wired: switching mode in Custom Settings rebound the provider, but the toolbar date picker drove nothing, no banner showed, editing was not locked out, and there was no AppData round-trip — so a reload came back live with no sign anything had been asked for. Both containers now share `useContainerHistoricalDate`, which owns the toolbar date, the as-of date, the AppData read/write (`historicalDateAppDataRef`) and the three derived values the grid reads (`toolbarDateHistoryEnabled`, `historicalViewMode`, `historicalViewMessage` — the last two are what disable editing and paint the banner). Picking a past date requires a configured historical provider (otherwise it reports through `onError` and changes nothing), writes the date through to AppData, flips the persisted mode, and queues a reload that carries `{ asOfDate }` through `provider.restart`. Returning to today leaves historical mode and reloads live. On mount, a PAST date in AppData restores the view; a stored "today" or a future date is refused, so a stale entry cannot restore a historical banner over live rows. What stays per container is the reload GATE — the client-side one waits for a live `GridApi` so its snapshot listeners are attached before `restart()`, the server-side one waits for a started provider — and `consumePendingReload()` is what each gates
- **Cold-start arbitration is shared** (`resolveProviderStartPlan`): a provider's hub slot is shared by every window, so a window restoring into historical mode attaches to a running slot and only restarts a cold one with the as-of overlay, giving a peer ~2 s (`PEER_PROVIDER_WAIT_MS`) to finish starting it first — a restart re-snapshots upstream for every window attached. A live cold start never waits and never restarts. This lived inline in `useProviderDataWiring`; the SSRM container had no arbitration at all, so a reloaded historical window attached to whatever the plane happened to hold. The server-side container passes the decision in as `useSsrmProviderDataWiring`'s `startProvider` override, because the rule needs the hub client and the resolved provider id, which the container holds
- **A provider's declared columns reach an SSRM grid intact.** `useSsrmColumnResolution` has ONE mapping path (`toSsrmColumnDefs`) for declared and inferred columns alike, and it hands them to the same `buildColumnDefs` `MarketsGridContainer` uses — so one expression compiler, one filter default and one nested-path rule serve both row models. The declared path used to cherry-pick five members (`field`, `headerName`, `width`, `hide`, plus the grouping capabilities) and drop `cellDataType`, `valueGetter`, `valueFormatter`, `cellRenderer`, `filter`, `sortable`, `resizable` and `type`. Consequences, all closed: a DSL `valueGetter` never reached the compiler so the column rendered its raw field value; every column fell to `agTextColumnFilter` because the multi-filter's first tab is chosen from `cellDataType` and there was none — which meant **declaring your columns produced worse typing than not declaring them**, since the inferred path did set it. The inferred path had the opposite hole: it never called `buildColumnDefs` at all, so inferred columns got no multi-filter envelope (and `withSsrmSetFilterValues` then attached the worker-backed whole-domain values to an envelope that was not there) and a dotted inferred field got AG-Grid's native dot-walk instead of the accessor that can tell `row.a.b` from `row['a.b']`. A column that declares its own `filter` still keeps it — a host or FilterEditor choice is never overridden. Every SSRM column additionally gets `enableRowGroup` / `enablePivot` / `enableValue`, because the query plane groups, aggregates and pivots server-side; that is a deliberate divergence in SSRM's favour, since a client-side grid leaves those to the host
- **SSRM column inference** — when the provider config declares no `columnDefinitions` (createStarui drafts, hand-seeded rows), `useSsrmColumnResolution` samples one block (`provider.getRows({ startRow: 0, endRow: 50 })`) after ready and infers `ColumnDefinition`s via `@wellsfargo-starui/data` `inferFields` (which then take the same mapping path as declared ones): `__`-prefixed internals and `object`/`array` fields are skipped, `cellDataType` maps from the inferred type, headers humanize the field name (`positionId` → `Position Id`). A declared list always wins (no sample fetch); inference resets on provider rebind. Previously an undeclared column list rendered a headerless, cell-less grid. The hook also resolves the row-key column and the cache block size — the three things a host may NOT override because they are facts about the provider
...
- `filtersToolbarLogic` — filter parsing + AG Grid model translation (label synthesis, pretty-printing, model equality / merge / subtract). The row-match predicate is NOT here — see `filterPredicate` below
...
| **grid-state** | explicit Save capture of live AG state | column width / order / pinned / visibility (replayed after all structure modules), plus sort / filter / pagination / selection and the viewport anchor. Every replay is **retried**, never single-shot: `setState`, `applyColumnState` and the viewport anchor each re-attempt across the cold-mount window (microtask → `firstDataRendered` → `modelUpdated`) because an SSRM grid's columns are inferred asynchronously and its row model only reaches the saved row index once blocks stream in — a single attempt silently no-ops there while CSRM succeeds. The viewport retry is bounded (20 attempts / 10 s) and stops as soon as the anchor is on screen, so an unreachable anchor never fights the user's scroll |
...
- **Alerts** — expression-driven notifications (dataChange / relativeChange /
  ...
  **ROW_ADDED / ROW_REMOVED are dispatched from an id-set diff only where the
  ids a full pass can see span the DATASET** — the module asks
  `platform.data.capabilities.canAddressUnloadedRows`, never which row model
  is mounted. Where they do not (a grid paging blocks in and out as the user
  scrolls), the diff would report every scroll as a burst of arrivals and
  departures, so the pass does no id bookkeeping at all: real arrivals and
  departures arrive on the transaction delta, and the previous-value baselines
  of rows that merely scrolled out are KEPT — dropping them is why a genuine
  change across a scroll-out and back used to re-seed silently instead of
  firing.
...
- `SsrmMarketsGridContainer` — full `MarketsGrid` chrome wired to an SSRM provider (`SsrmMarketsGridContainerProps`); the SSRM counterpart of `MarketsGridContainer`, accepting the same host surface (see the SSRM row-model section for the prop-by-prop reading and the members that stay mode-specific)
- `useSsrmProviderDataWiring` — provider start/ready wiring behind `SsrmMarketsGridContainer` (`UseSsrmProviderDataWiringParams`)
- `useSsrmColumnResolution(provider, ready)` — `{ keyColumn, columnDefs, cacheBlockSize }` derived from the provider's resolved config (declared defs, else sampled inference). **Internal** to the SSRM container
...
- Internal machinery reachable through a `GridPlatform` instance (no longer
  ...
  and taking the same delta by hand through `transactionApplied` from
  whoever applied a transaction AG-Grid does not announce — the server-side
  tick binding and `SsrmDataAdapter.mutate`, since `applyServerSideTransaction`
  fires no flush event and leaves `modelUpdated` as its only trace; an EMPTY
  report is not treated as a delta, so a refused transaction cannot downgrade
  a coinciding structural change. One `RowChange` per frame with
  `added`/`updated`/`removed` deltas or `full` for sort/filter/`setRowData`/a
  server-side block refetch, so data-reactive modules evaluate only changed
  rows under EITHER row model; filter pill counts adjust incrementally on
  deltas. The write half is `RowChangeSink`, deliberately absent from the
  `RowChangeSignal` modules receive),
...
- **Calculated columns:** `buildVirtualColDef`, `getAllRowsSnapshot`, `fillAllRowsSnapshot`, `invalidateAllRowsCache`. The cross-row snapshot an aggregate expression folds over is a pure cache — `getAllRowsSnapshot` never walks the row model itself. The module's `activate()` fills it through `platform.data.scan`, gated on `capabilities.canAddressUnloadedRows`: where a walk of every row is meaningful it is the same `forEachNode` pass the module always made (and the client-side adapter visits synchronously inside the call, so the first paint is unchanged); where it is not, walking would page a whole server-side dataset across `postMessage` to rebuild a fold the source has already done, so it does not, and the source's stamped values answer instead. A scan that could not cover its scope puts the previous snapshot back rather than leaving a partial one
...
- `doesValueMatchFilter()`, `doesRowMatchFilterModel()`, `assertFilterModelSupported()`, `compareValues()` — re-exported from `@wellsfargo-starui/core`'s ONE filter predicate (see below); `filtersToolbarLogic` no longer carries its own
...
#### The filter predicate — one implementation

`packages/core/engine/src/filters/filterPredicate.ts`. **"Does this row match
this filter model" exists exactly once in the tree.** Three readings used to:
the SharedWorker query plane's `rowPassesFilter`, `filtersToolbarLogic`'s own,
and AG Grid's. The first two are this module, consumed by the query engine
(`@wellsfargo-starui/data` re-exports rather than reimplementing), both
`GridDataPort` adapters, the filter-pill badges and the server-side tick
fan-out alike.

- `doesRowMatchFilterModel(row, filterModel)` — the walk. Handles the COLUMN
  MAP and the Advanced Filter TREE (mutually exclusive in the request AG Grid
  builds), multi-filter envelopes, combined condition lists (modern
  `conditions` and legacy `condition1`/`condition2`, so persisted profiles keep
  loading), and dot-path column ids via `getPathAccessor`
- `doesValueMatchFilter(value, filter)` — one column's entry against a value
  already in hand. Same walk, so it cannot disagree with the row form
- `assertFilterModelSupported(filterModel)` — the same walk with no value
  source: resolves every operator, evaluates nothing. Callers that scan run it
  once up front so the verdict reads the request, never the rows
- `compareValues(a, b, dir)` — the sort half of the same value coercions, so a
  sort cannot type `'20'` differently from the filter that admitted it
- `UnsupportedQueryError` — the refusal. Nothing falls through: an operator or
  shape the predicate cannot evaluate is refused with user-facing copy naming
  the limit and the alternative, never substituted or widened to every row

Seven behavioural differences between the two old predicates were resolved
deliberately (SSRM parity roadmap Phase 2); each is pinned by name in
`filterPredicate.test.ts`:

| # | Behaviour | Now |
|---|---|---|
| 1 | set filter `values: []` | matches NO rows (what the grid shows) |
| 2 | text `blank`/`notBlank` | trims, matching AG Grid's own `isBlank` |
| 3 | date filters | evaluated (the client copy matched everything) |
| 4 | Advanced Filter tree | walked (the client copy matched everything) |
| 5 | unknown operator | refused (the client copy returned true) |
| 6 | `''` on a number column | blank (the client read `Number('')` as 0) |
| 7 | multi-filter join | ANDs, and honours an explicit `operator` |

Callers decide what a refusal means, per site rather than by blanket catch:
the port adapters report it as `complete: false` (the channel that already
separates "found nothing" from "could not look"), while the two hot paths that
must not drop work — `bindSsrmTicks`'s tick fan-out and the pill-count
badges — OVER-include and warn once, leaving the query path to raise it for
real where it reaches the user.
...
- `AdoptedPort` / `installSharedWorkerHub({ adoptPorts })` — port handover for entries that must accept connections before the hub exists. `defaultEntry` cannot build its ConfigManager until the handshake arrives, so it buffers each port's messages and replays them through the hub, in arrival order, on adoption — no early `appdata-attach` is lost. The handover is **gap-free**: `AdoptedPort.release()` detaches the caller's temporary listener and is invoked synchronously immediately before the hub attaches its own, with `buffered` read as a live array in the same turn. Releasing earlier lost messages outright — the adopted port is already `start()`ed, and a started MessagePort discards (not re-queues) messages arriving with no listener, while `installSharedWorkerHub` awaits `hydrateCatalog()` + `hydrateAppData()` in between. Clients post `appdata-attach` immediately on connect, so that is exactly what used to vanish, hanging `AppDataMirror.ready()` forever. Port messages arrive as tasks, so nothing can slip between two synchronous statements — never introduce an `await` into that loop
...
- **Nothing falls through the query engine** (`UnsupportedQueryError`, defined in `@wellsfargo-starui/core` and re-exported from `./ssrm-engine` and the runtime barrel — one refusal type behind one predicate, so `instanceof` holds whichever side raised it) — a filter option, filter shape or `aggFunc` the plane cannot evaluate is REFUSED, with user-facing copy naming the limit and the alternative, instead of silently substituting another (`contains` for an unknown text option, `equals` for an unknown number one, `sum` for an unknown aggregation) or widening to every row. `assertFilterModelSupported` runs the check ONCE per query before any row is scanned, so the verdict reads the request and never the data — an empty store refuses exactly what a full one does; validation and evaluation are one walk (no value source means validate-only), and the combinators are non-short-circuiting so a bad condition beside a satisfied one is still reached. The refusal crosses the worker boundary as the RPC's `error` string and reaches `params.fail()` in `createSsrmDatasource`. AG Grid's relative-date presets (`today`, `last7Days`, `thisQuarter`, …) are the one family refused rather than evaluated — they are not in `DEFAULT_DATE_FILTER_OPTIONS`, and evaluating them would mean re-deriving the grid's own week/quarter boundaries
- **Advanced Filter evaluates server-side** — AG Grid sends an `AdvancedFilterModel` TREE in place of the column map when the feature is on (`isAdvFilterEnabled() ? getAdvFilterModel() : getFilterModel()`), and the plane now walks it: `join` nodes (AND/OR, nested) plus every `ColumnAdvancedFilterModel` variant — `text`, `object`, `number`, `bigint`, `date`, `dateString`, `dateTime`, `dateTimeString`, `boolean`. It previously fell through `Object.entries` to a catch-all `return true`, i.e. an advanced filter returned the entire unfiltered dataset
- **Nested-path columns work throughout the plane** — filter, sort, group, pivot, aggregation, set-filter values, tree parent/key/path fields and quick-filter all read values through the repo's cached path accessor (`getPathAccessor`, the compiled sibling of `getValueByPath`), so a column whose field is `quote.bid` matches the nested value the projector kept (`providers/fieldProjection.ts` preserves real sub-objects). A flat `row[colId]` read found `undefined` for every one of them. Sort accessors resolve once per sort entry rather than per comparison, which also makes a cold 100k sort measurably faster
...
- SSRM aggregations return **blank (`null`) rather than `0`** for `min` / `max` / `avg` over a group where no row has a value (`aggregations.ts`); `count` still counts to 0 and `sum` keeps its additive identity. A 0 price or 0 spread is real data, so 0 could not mean "no values". `computeStatusBar` carries that `null` through (`StatusBarAggValue.value` is `number | null`) instead of finishing with `Number(value ?? 0)`, and folds once per DISTINCT aggregation so a panel asking for MIN(px) and MAX(px) gets two answers rather than one written twice into a field-keyed row
...
- **Column-wide aggregates fold the whole row store.** A `calculated` rule whose expression calls an aggregate function (`SUM([px])`, `AVG([mid])`, `MEDIAN`, `STDEV`, `DISTINCT_COUNT`, …) is not row-local, and enrichment used to hand the evaluator the row it was enriching — so `SUM([px])` quietly returned *that row's* px. `QueryEngine.aggregateScope(sessionId)` supplies the evaluator's `allRows` plus its two memos, built from `store.iterate()` and bound to the store REVISION, so a tick rebuilds it exactly once and every row of every block in between shares one pass per column. Scope is the whole store, **unfiltered** — the client-side row model's `forEachNode` snapshot is unfiltered too, and the same expression must not mean "total of everything" in one row model and "total of what's showing" in the other. A session whose rules are all row-local never builds it (`ExpressionRuleStore.usesAggregates`, decided once at configure time), so a grid with no aggregate column pays nothing
- **The fold's RESULT is memoised, not just its input.** `EvaluationContext.allRowsAggregateCache` (shared by the interpreter and the compiled closure through `evalOps.evaluateCall`) keys a whole-column fold by function + column ids, and only for calls whose arguments are ALL direct column refs — those are row-independent by construction, so one answer is every row's answer. `[px] / SUM([px])` still varies per row; its `SUM` sub-call does not. Without it a 100-row block re-reduced a 100,000-element array once per row: measured **223 ms per block, and 1.7 ms with it** — parity with a row-local column. The client-side calculated-columns module carries the same pair on its `AllRowsEntry`, so both row models pay the fold once per snapshot generation
- **Enriched rows name what the plane computed** — `__ssrmCalculated` lists the `calculated` fields present on the row (one frozen array shared by every row of every block, so the stamp is a property write and not an allocation). The grid returns those values instead of re-evaluating the expression against its block cache; see the SSRM grid section
...
- `AppDataMirror`:
  ...
  - `ready()` promise + `subscribe()` reactivity — `ready()` resolves ONLY from the hub's `appdata-snapshot`, so `attach()` retries the handshake (`APPDATA_ATTACH_RETRY_MS` 2 s × `APPDATA_ATTACH_MAX_RETRIES` 5) until that snapshot lands rather than firing once and hoping. A single dropped `appdata-attach` previously hung `ready()` — and every caller awaiting it — permanently. Safe to repeat: `HubAppDataService.handleAttach` is idempotent (overwrites the listener entry, re-posts the snapshot)
  - `dispose()` — stops the attach-retry timer; called by `detachAppData()` and `close()` so a mirror torn down mid-handshake stops posting into a dead port
