...
- **Calculated columns** — virtual cols from expressions; under SSRM the editor names the column's tier (`SSRM TIER` chip + note: grid-computed per loaded row, sort/filter/group locked — the engine has no expression support; `SUM`/`AVG`/`MIN`/`MAX`/`COUNT` flagged as reading engine-wide totals via `astUsesAggregateFunctions`), so nothing is silently blank
