...
- `./customizer` — hooks (`useEditJournal`, `useModuleState`, `useProfileManager`,
  ...
  grid-state capture/restore (`captureGridState`, `applyGridState`, `restoredExpandedGroupIds` — capture derives SSRM `rowGroupExpansion` from loaded group nodes since AG Grid's state module doesn't report it server-side, and apply stashes the ids on the api for the SSRM surface's `isServerSideGroupOpenByDefault`, because `setState` cannot expand rows that haven't loaded),
...
- `MarketsGrid` — main grid component (host integration, column defs, real-time rows); optional `ssrm` prop remounts as AG Grid 36.1 `rowModelType="serverSide"` (`createSsrmDatasource`, `bindSsrmTicks`, `watchGroupsFromApi`, required `getRowId`). `MarketsGridSsrmSurface` sets `suppressServerSideFullWidthLoadingRow` and `SsrmBlankLoadingCellRenderer` so SSRM block placeholders stay blank (no default "Loading…" text or skeleton cells), answers `isServerSideGroupOpenByDefault` from the profile-restored expansion stash (`restoredExpandedGroupIds`, below) so a reload re-opens the saved groups as their rows load, and sets `serverSidePivotResultFieldSeparator` to the engine's `|` so pivot result fields split where the engine joined them
- `SsrmBlankLoadingCellRenderer` — no-op AG Grid SSRM loading cell renderer used by `MarketsGridSsrmSurface`
- `createSsrmDatasource` / `bindSsrmTicks` / `bindSsrmExpressionAggregates` / `ssrmGetRowId` / `watchGroupsFromApi` / `withSsrmStatusBar` / `useSsrmStatusBar` / `applySsrmStatusBar` — AG Grid 36.1 SSRM helpers (`success({ rowData, rowCount, groupData?, grandTotalData? })`, `applyServerSideTransactionAsync`, `refreshServerSide`). `bindSsrmTicks` applies every tick in place with `applyServerSideTransactionAsync` (`update` for loaded rows, `remove` for `removals` as `SSRM_ROW_ID_KEY` id stubs) and reserves `refreshServerSide` for position changes: a loaded row whose sort / filter / group key changed refreshes on `refreshThrottleMs` (250 ms), unloaded rows ticking under a sort / filter / grouping (and group aggregates) refresh on `positionalRefreshMs` (1 s), and in a flat view unloaded upserts / removals trigger a `getRowCount` reconciliation on `countCheckMs` (1 s) that refreshes only when the engine total moved — the former "any sort ⇒ refresh every loaded block per tick batch" storm is gone. Refreshes are deferred while `bodyScroll` is firing and while a paste is in progress (`pasteStart` / `pasteEnd`, `scrollResumeMs` 150 ms) so viewport block reads and cells being written never queue behind re-reads. Loaded rows are indexed once per tick with `forEachNode` (AG Grid's SSRM `getRowNode` is a linear scan, quadratic at trading rates). Failed blocks are retried (`retryServerSideLoads`) on `ready`, on every positional refresh and on the count-check timer. Also purges on `ISsrmDataProvider.onRefresh` and on `ready`. `createSsrmDatasource` forwards the live `quickFilterText` into each block request (AG Grid's own quick filter is client-side only), holds only SET-filter slots off the first block (text / number / date conditions and the quick filter ride it, so a restored filter paints and counts correctly from the first block), holds reads while `provider.status` is `loading` (`readyTimeoutMs` 60 s) so the first paint is the snapshot rather than an empty grid with row count zero, retries a block read (`maxAttempts` 3, `retryBackoffMs` 250) with a per-attempt `requestTimeoutMs` (15 s) before `fail()` — AG Grid never retries a failed block on its own — and warns once per distinct `unsupportedFilters` list the engine reports (the block then holds MORE rows than the filter asks for). With a `cache` (`SsrmBlockCache`, shared with `bindSsrmTicks`) a warm block is served in the same frame AG Grid creates the stub rows, so they paint once with data; the served block is re-read in the background and drifted rows patched by id, and `prefetchBlocks` (1) blocks ahead in the scroll direction are warmed after every served block
- `SsrmBlockCache` / `ssrmViewKey` — client block cache keyed by view signature (sort, filters with key order normalised, quick filter, group path, value / pivot columns) + block start; LRU (`maxBlocks` 64), `ttlMs` 10 s, rows indexed by id so `patchRows` can overwrite one row in place. `bindSsrmTicks` patches it on updates and clears it before anything that can move rows (refresh, purge, removals) — it caches a fixed view and is never a second source of truth
- `bindSsrmEdits` / `ssrmPasteTarget` — cell edits, clipboard pastes and fill-handle writes are coalesced per row id (`flushMs` 16, flushed at once on `pasteEnd`) and written back through `ISsrmDataProvider.applyEdits` as whole rows PLUS the per-row `editedColumns` the user actually touched (read via `event.column.getColId()`), so they survive the next tick and block refresh, reach every grid on the provider, and the worker can hold exactly the edited cells over whole-row upstream resends (see `SsrmWasmPlane` edit overlays); a provider without `applyEdits` leaves edits local (the previous behaviour). `ssrmPasteTarget` resolves where a paste would land (selected ranges, else downward from the focused cell) and counts block placeholders / group rows; `MarketsGridSsrmSurface` wires it as `processDataFromClipboard` and refuses (returns `null`, warns) a paste that would hit unloaded rows rather than land a partial paste that looks complete. The surface also passes `getChildCount` (engine group rows carry `__count`) so group rows show "(n)" `bindSsrmExpressionAggregates` caches `getAggregates` for expression aggregate scalars — `SUM` / `AVG` / `MIN` / `MAX` / `COUNT` and, since plan §12 T4, `MEDIAN` / `STDEV` / `VARIANCE` / `DISTINCT_COUNT` — so calculated columns and provider `valueGetter`s (`[col1] / SUM([col1])`) use the engine total for the current filter, not the loaded cache blocks. `withSsrmStatusBar` swaps the built-in status panels (total / filtered / selected / aggregation) for provider-backed React panels that keep AG Grid's `ag-status-name-value` chrome. `useSsrmStatusBar` holds that remapped object stable across pipeline ticks that don't change the enabled panel set, and keeps the last bar when the pipeline omits the key (omit is not "off"). `applySsrmStatusBar` is the only live write — `MarketsGridSsrmSurface` does not pass `statusBar` as an AgGridReact prop (that tears the bar down / fails to show it after mount). Grid Options `statusBar` / `statusBarShow*` always emit a `statusBar` object (`statusPanels: []` when off) so visibility is deterministic. Aggregation labels stay mounted (`–` until a value arrives) so the bar height does not pump. `useGridHost` skips `setGridOption('statusBar')` in SSRM so the raw `ag*` panels cannot fight the remapped bar
- `SsrmTotalAndFilteredStatusPanel` / `SsrmFilteredStatusPanel` / `SsrmTotalStatusPanel` / `SsrmSelectedStatusPanel` / `SsrmAggregationStatusPanel` / `useSsrmStatusModel` — SSRM status-bar panels. Counts and aggregations read `ISsrmDataProvider.getRowCount` / `getAggregates` (the engine holds the dataset), tick-gated: the 1 Hz cadence only re-reads after a provider tick / refresh / status change arrived, so an idle feed costs zero counting RPCs (grid-side changes — filter, selection — still refresh immediately, and the cadence keeps retrying while the first answer is outstanding). Count panels render an en-dash until the engine has answered once (`model.loaded`) instead of a confidently-wrong `0`; the Selected panel never dashes (selection is grid-side, `0` is honest). Selected-row count reads `getServerSideSelectionState` — a header select-all is the filtered total minus the un-ticked ids, the toggled ids are the selection otherwise, and a `groupSelects` tree is counted through `countGroupSelection` (toggled entries mean "opposite of parent", per AG Grid 36's own serializer; group populations come from loaded rows' engine `__count`) — falling back to `getSelectedNodes` only when a toggled group's row is not loaded. One poll is shared across every panel on the same grid. Same panel set, labels, and `ag-status-name-value` chrome as CSRM's `agTotalAndFilteredRowCountComponent` / `agFilteredRowCountComponent` / `agTotalRowCountComponent` / `agSelectedRowCountComponent` / `agAggregationComponent`; honours CSRM's `aggFuncs` and `valueFormatter` statusPanelParams
- `withSsrmSetFilterValues` / `withSsrmSetFilterDefaults` — supply `filterParams.values` for set filters under SSRM, which can't derive a list without client-side rows. Walks colDefs (column groups + `agMultiColumnFilter` set slots) and the `defaultColDef` separately, since AG Grid doesn't merge `filterParams` between them; `filter: true` counts (Enterprise resolves it to the set filter). A Multi Filter with no `filters` array gets AG Grid's default composition materialised (type filter by `cellDataType` + Set Filter) — otherwise there is no colDef to hang `values` on, and the stream-safe floating filters have no sub-filter slots to route models through. Adds `refreshValuesOnOpen` (live feed) and `suppressClearModelOnRefreshValues` (keep the selection); an explicit `values` list always wins. The lookup carries the other columns' filter model AND the live `quickFilterText`, so the list never offers values the grid would show zero rows for. The lookup is bounded by `VALUES_TIMEOUT_MS` (5s, overridable per call) and falls back to an empty list — load-bearing rather than defensive: AG Grid won't apply a set-filter model until the list arrives, so an active saved-filter pill puts this lookup in front of the FIRST BLOCK, and a lookup that never settles (saturated worker, or one left from a previous page load that predates the RPC) is a grid that never shows a row. The selection survives the empty list, so the pill still filters. Applied automatically by `MarketsGridSsrmSurface`
- `compileSsrmComputedColumns` — walks the (transformed) columnDefs for `staruiVirtual` columns carrying `context.staruiExprSource`, classifies each source under the engine expression contract, and compiles every tier-`compiled` one to a `SsrmComputedColumnSpec`; `MarketsGridSsrmSurface` rides the result on every `getRows` request (`createSsrmDatasource.getComputedColumns`) and feeds the engine-backed colId set to `lockSsrmExpressionColumns`, which skips them — sort / filter / row-group / aggregation on those columns run in the WASM engine dataset-wide (plan §12 T3)
- `bindSsrmAlertPredicates` (alerts runtime) — every enabled, un-column-scoped `dataChange` rule whose expression compiles is registered as an engine predicate watch (`ISsrmDataProvider.watchPredicate`); the engine diffs the predicate's row set per revision and pushes entered rows as `viewDelta` ticks, dispatched through the same `createAlertDispatcher` as client hits, so alerts fire over the WHOLE book, loaded or not (plan §12 T5). Engine-watched rules are excluded from the client evaluator (no double fire), deltas are pinned to the registering subId (two windows never both fire), and rules outside the grammar (diff refs, column-scoped, relativeChange) stay on the loaded-rows path
- `attachSsrmSession` / `getSsrmSession` / `isSsrmGrid` / `drainSsrmRows` / `exportSsrmVisualExcel` / `lockSsrmExpressionColumns` / `withSsrmSelectAll` / `sendSsrmClipboard` / `wrapSsrmContextMenu` — SSRM honesty locks. Excel/CSV drain the filtered book from the engine (refuse above `SSRM_EXPORT_MAX_ROWS`) instead of `exportDataAsExcel` on loaded blocks, and a selection export under header select-all excludes the `toggledNodes` the user un-ticked, and a `groupSelects` selection tree filters the drained flat rows by each row's rebuilt group chain (`filterRowsByGroupSelection` — level ids in the `key` / `level:parents:key` shape `createSsrmGetRowId` mints); calculated / expression columns (`context.staruiVirtual` / `staruiExpression`) OUTSIDE the engine expression grammar cannot be sorted, filtered, or grouped — tier-`compiled` ones are engine-backed (`compileSsrmComputedColumns`) and stay fully live; Advanced Filter is forced off (`toViewSpec` has no tree compiler); multi-row header select-all sets `selectAll: 'all'` so AG Grid 36 uses `setServerSideSelectionState`; clipboard still copies loaded text and says so. Smart Edit, Bulk Update, plus-minus, and letter-key shortcuts enable only when the SSRM edit writer is attached (a provider with `applyEdits` — their writes then persist engine-side via the applyPatches seam); without one the honest disables stand. Conditional-styling matching paints loaded (visible) rows (labelled); compiled alert rules and aggregate styling thresholds are book-wide (plan §12 T5 + the styling aggregate context)
...
- **Smart Edit** — bulk update, arithmetic across cell selections (× ÷ + −),
  ...
  `SmartEditToolbarBody` in `@wellsfargo-starui/grid`. Under SSRM: enabled when the engine edit writer is attached (writes persist via ssrm-apply-edits), honestly disabled otherwise. Settings panel: **Smart Edit**.
...
- **Bulk Update** — replace all selected cells in one column with the same
  ...
  single-column guard, journal integration. Under SSRM: enabled with the engine edit writer, disabled otherwise. Settings panel: **Bulk Update**.
...
- **Plus / Minus** — keyboard +/- nudge rules with per-column increment/decrement
  ...
  integration via `recordHistory`. Under SSRM: works with the engine edit writer, no-ops otherwise. Settings panel: **Plus / Minus**.
...
- **Shortcuts** — letter-key arithmetic (× ÷ + −) with per-shortcut operand and
  ...
  Under SSRM: works with the engine edit writer, no-ops otherwise. Settings panel: **Shortcuts**. Lab: **Shortcuts** tab (`lab-shortcuts`).
...
- `apps/source/markets-grid-lab-ssrm` — the Feature Lab's 17 tabs against the server-side row model (`:5301`): the lab's OWN `LabFeatureConfig`s (imported, so parity cannot drift) mounted through `MarketsGrid.ssrm` on one shared `mock-ssrm` provider — same worker generator as the CSRM lab, no broker. Home is a feature-by-feature parity matrix (full / partial / gap with the mechanism behind every gap, `src/parity/parityNotes.ts` — after plan §12 T3–T7 + C1/C2: 14 full / 2 partial / 0 gap — the partials are the deliberate viewport semantics, conditional styling and raw client valueGetter renderer columns); each tab repeats its verdict above the grid. Demo console pauses/paces the feed and resizes the book via `provider.restart` (a SHRINK shows exactly the new snapshot — T2 `replace_snapshot`; live-verified 500→100→2,000) and injects the lab's scenarios through `ssrm-apply-edits` over the loaded blocks; demo profiles seed against the `-ssrm` grid ids (the storage adapter is gridId-scoped); client-synthetic valueGetter columns are branded `staruiSsrmClientExpr` so the honesty lock disables their sort/filter/group; the engine boot schema types every lab field with a drift-failing test
- `apps/source/spg-pricing-blotter` — a structured-products trader blotter over SSRM with a REAL server (`:5401`; `npm run dev` starts both): plain-Node server (`node:http` + `ws` + `node:sqlite`, zero native deps) speaking the same STOMP contract as `stomp-view-server`, SQLite seeded from a committed 2,500-row file, REST write path (`/api/updates` per-row verdicts, `/api/lookup`, `/health`). The demo's spine is the write lifecycle: ONE wrapper over `ISsrmDataProvider.applyEdits` (`withServerWrites`) gives every grid write path — cell edits, multi-hundred-row pastes, Smart Edit/Bulk Update, CSV-import Save — the same commit lifecycle, painted per cell from a `CellStateStore` via `cellClassRules`: amber = staged locally (CSV import before Save), yellow border = at the server awaiting commit (the server acks after a configurable delay so the window is visible), clear + flash = committed (the flash is the server's own post-commit feed echo, carrying re-derived `marketValue`/`priceChangePct`), red = refused. CSV import validates against the server first (unknown cusips can never upsert phantom engine rows), previews old→new, stages amber, and commits via Save / rolls back via Discard (server-row restore). Live-verified end to end incl. the pending→confirmed timeline
- `apps/source/stomp-ssrm-minimal` — `HostedSsrmMarketsGrid` against one catalog row; auto-starts `stomp-view-server` via `npm run app -- stomp-ssrm-minimal` (the launcher spawns through a shell and kills with `taskkill /T` on Windows). `?rate=<updates/sec>` on the demo URL sets the broker's live trigger rate for load runs (default 1000). The stored provider row is re-saved ONLY when the built config actually differs from the stored one (a byte-equal re-save restarts the provider and re-streams the 20k snapshot — most of the old 5–42 s cold start); the config declares no `searchColumns` (quick search covers every text column), enables pivoting from the columns tool panel (`enablePivot`), and carries a `maturityDate` `dateString` column plus editable `trader` / `marketValue` / `pnl` / `currentPrice` for paste tests. `apps/scripts/ssrm-perf/` holds the Playwright measurement harnesses and WASM engine probes behind the SSRM baselines (README inside; run against `vite preview`, not the dev server), including `ssrm-multiwindow.mjs` — N same-origin pages sharing one SharedWorker / STOMP session / engine cache, measuring per-page cold start, idle RPC volume, sibling-scroll isolation, and cross-window edit propagation. Same chrome as the CSRM demo: filters row, formatting toolbar (which also enables the View menu's Auto Format item), editing toolbar
...
- `ColumnDefinition.editable` — per-column cell editing on the provider contract. `buildColumnDefs` hands a provider's `columnDefinitions` to AG Grid as `ColDef`s verbatim, so this has always been honoured at runtime; it is now on the type, which is what `stomp-ssrm-minimal`'s editable `trader` / `marketValue` / `pnl` / `currentPrice` columns need to typecheck
...
- `appendLaunchIdentityParams` — now exported from `@wellsfargo-starui/openfin` and its `/config` subpath, so the Workspace Setup registry editor can stamp the same launch identity onto a "Configure Component" URL that `launchRegisteredComponent` stamps onto a dock launch. `customData` reaches a view asynchronously; a component that reads its id from the URL sees nothing without these params
...
- `DiagnosticsTab` — probe, request/response logging, debug; Snapshot card shows fetch time and row count; under CSRM also "Cache size (serialized)" (`stats.cacheBytes`, the worker-cache footprint that `projectFields` shrinks). Under **SSRM** the rows live in the WASM engine rather than the worker cache, so "Rows loaded" comes from the engine (`mem_stats().datasources[].cacheRows`, surfaced by `SsrmWasmPlane.engineStats`) and the byte figure is replaced by "Engine subscribers" / "Engine views open" — the engine reports counts, never bytes, so `cacheBytes` is left undefined rather than rendered as a misleading `0 B`; Connection latency card shows "Restart → request sent" (`stats.restartRequestMs`, click-to-upstream-request including dial + handshake) and "Request → first message" (`stats.firstMessageMs`, request-sent to first upstream frame); Throughput card's byte stat is labelled "Bytes received" (upstream wire traffic, unaffected by projection)
...
- `BehaviourFields` — per-transport behaviour knobs. Every transport: a **Start-up** section with the `autoStart` switch ("Start with the platform") — the OpenFin dock / app warm-up (`warmPlatform(..., { providers: 'autoStart' })`) starts the provider in the data worker before any view asks for it. **`stomp` and `stomp-ssrm` share the same sections**, because they share the transport (`registry.ts` maps both to `startStomp`, which never branches on `providerType`): reconnect initial delay, realtime throttle (on/off switch + ms) + conflation (on/off switch + key column), snapshot chunk size, and `projectFields` row pruning. Each acts at ingest, upstream of the worker cache (CSRM) or the WASM engine (SSRM). SSRM omits exactly two — `thinDeltas` and `wireFormat` — because both shape the worker→client ROW broadcast that an SSRM grid never receives; the tab says so rather than hiding them silently. All labels are associated with their controls (`htmlFor`/`id`). Other transports: the Start-up section only
...
- SSRM engine feature surface (data): `SsrmGetRowsRequest.computedColumns` (engine computed columns per request, gated on the wasm `capabilities()` manifest — an engine build without `computedColumns` gets the spec stripped and the block reports it unsupported), `watchPredicate` / `unwatchPredicate` RPCs (`ssrm-watch-predicate` / `ssrm-unwatch-predicate`) + `viewDelta` tick payloads (`ruleId`, `entered` / `left` keys, entered rows, `watchSubId` routing), typed date columns (boot schema `type: 'date'` — the engine parses epochs at write; the client `__epoch` shadow machinery is deleted, date filter bounds and sorts target the real column), and group-less pivot (grand-total row; the engine folds `|`→`¦` inside split values)
- FieldSelector types
- Configuration types
...
- `ExternalFilterColumnRegistry` (`platform.externalFilters`, type `ExternalFilterColumns`; also on every module's `PlatformHandle`) — where a module that installs an AG Grid external filter declares the columns its `doesExternalFilterPass` reads (`declare(owner, cols | null)`, `columns()` = union or `null` when nothing is declared). AG Grid cannot attribute an external filter itself; with a declaration the rendered-row apply path treats only those columns as position keys while the filter is active, without one every updated row rides a transaction. Cleared on `destroy()` (plan B2)
- `RowChangeBus` (`platform.rows`, types `RowChangeSignal` + `RowChangeFeed`) — shared, timer-coalesced row-change emitter. Reads the exact changed nodes from AG `asyncTransactionsFlushed`, and from `noteRowsChanged(nodes)` for rows the rendered-row apply path updates without a transaction (counts as a flush for the frame's delta/full classification), and emits one `RowChange` (`added`/`updated`/`removed` deltas, or `full` for sort/filter/`setRowData`; explicit `sortChanged`/`filterChanged` listeners keep the `full` classification even when the sort/filter shares a coalescing window with a streaming flush) per frame, so data-reactive modules (alerts, conditional-styling, filter counts) evaluate only changed rows instead of walking the whole grid on every streaming tick. Filter pill badge counts (`useFilterCounts` in `useFilterModel`) maintain per-filter row-id sets and adjust counts incrementally on delta emits
...
- **Editing core** — `EditJournal`, `CellPatch`, `EditSource`, `buildPatchesFromTargets`,
  ...
  SSRM grids now persist through the edit-writer seam (`applyPatches` writes ssrm-apply-edits alongside the server-side transaction),
...
- `collectColumnRefs(node)` — every `[column]` an expression reads, once, in first-seen order (the toolbar-date row exclusion declares these as its external-filter columns, plan B2)
...
- `compileToEngineExpression` / `classifySsrmExpression` / `toComputedColumnSpec` — the SSRM engine expression contract (plan §12 T1): DSL AST → the versioned `SsrmExprNode` wire grammar (`@wellsfargo-starui/types` `ssrmExpression.ts` — operators, IN/BETWEEN, unified `cond`, scalar fn set, `agg` nodes for `SUM/AVG/COUNT/MIN/MAX([col])`), with anything outside v1 REPORTED untranslatable (variables, diff refs, REGEX_MATCH, NOW/TODAY/DATE_DIFF/DATE_ADD, statistical varargs outside the `FN([col])` aggregate form) — never guessed. Semantics are the CLIENT evaluator's, coercions and all, pinned by 48 golden fixtures (`ssrmExpressionContract.fixtures.json`) and their GENERATED wire corpus (`ssrmExpressionContract.wire.json`, 44 compiled cases) — the corpus the Rust engine reproduces in its `expr_contract` conformance suite (regenerate + copy to rangrez `tests/fixtures/` when the fixtures grow). The `agg` node covers the FULL aggregate set (T4: sum/avg/count/min/max/median/stdev/variance/distinct_count); the classifier's two tiers (compiled / materialized) + `requires` feed the customizer chip
...
- **Editing core SSRM write hook:** `SSRM_EDIT_WRITER_KEY` / `attachSsrmEditWriter` / `lookupSsrmEditWriter` — GridApi brand the SSRM surface binds to `ISsrmDataProvider.applyEdits`; `applyPatches` persists every editing write through it alongside the local server-side transaction (whole rows + per-row edited columns, so the worker edit overlay holds exactly what was touched). Bulk Update, Plus/Minus, Shortcuts, Smart Edit and `EditJournal.undo/redo/undoTo` all funnel through `applyPatches`, so one seam re-enabled the whole family under SSRM (plan §12 C1/C2) — their honesty disables now lift exactly when a writer is attached, and an undo persists as an ordinary engine write of the old values
...
- `./runtime/worker/defaultEntry` — default worker entry; accepts ports itself, waits for the `worker-bootstrap` handshake, then builds the ConfigManager and installs the brain the SharedWorker's own `name` selects (worker-split): `mkt-platform-services:*` → `installPlatformServicesHost` after a full `ConfigManager.init()` (the **sole seeder**; catalog + AppData hydrate before port traffic); `mkt-data-services:*` → `installSharedWorkerHub` after a read-only `init({ mode: 'attach' })` (no seed URL, nothing to hydrate — provider cfg + AppData are read on demand at lifecycle moments)
...
- `SharedWorkerDataServicesClient` — main-thread client routing events to listeners; SSRM RPCs reject after `ssrmRpcTimeoutMs` (`DEFAULT_SSRM_RPC_TIMEOUT_MS`, 15 s; `0` disables) and every pending SSRM RPC is rejected on `close()`, so a lost worker reply can no longer leave an AG Grid block loading forever; catalog RPC (`waitForCatalogReady`, `getProviderConfig`, `listProviderConfigs`, `invalidateConfig`, `getHubIntrospect`, `isProviderRunning`, `waitForProviderRunning`, `onCatalogChange(detail)`); scoped `catalog-ready` broadcasts carry `providerId` (single row) or `full` (whole catalog); **Deprecated.** passing `cfg` on `attach` / `subscribe` for catalogued providers — use cfg-free attach
- catalog invalidation originates where the write lands (W2): `PlatformServicesHost` subscribes to its own ConfigManager's `onConfigChanged` — its `config-save` / `config-delete` refresh the catalog inline before replying (skipped by the listener via `ownWrites`), while writes from any other context (a no-worker fallback window, another machine in REST mode) arrive over the ConfigManager's cross-context channel — and only `data-provider` / `appdata` rows (`isCatalogConfigRow`) refresh and broadcast `catalog-ready`; the window-side `wireWorkerCatalogSync` is deleted
- no-SharedWorker fallback attach bootstrap — only where the platform-services worker cannot spawn: when cached seed identity (localStorage, cross-window) + `isPlatformWarm(appId)` hold, the window skips `seedConfigUrl` and runs `ConfigManager.init({ mode: 'attach' })`; on the normal path a window never seeds at all, and `markPlatformWarm(appId)` (set after full hydration) only feeds this fallback
- `ensureConfigReady(config, { workerScriptUrl? })` — the **thin-window** config tier (worker-split W2): spawns the platform-services worker FIRST and nothing else (`warmPlatformConnection`), opens the shared IndexedDB read-only (`init({ mode: 'attach' })`, no seed URL — the window never seeds, never fetches a seed bundle, never takes the seed lock) and gates `config-ready` on the worker's catalog (which implies it seeded) with a `CONFIG_READY_DEADLINE_MS` (20 s) backstop that warns and continues; the window's config writes ride the same port via a `ConfigWriter` (`config-save` / `config-delete`), so the services worker is the only context writing config rows; idempotent per `appId`; `ensurePlatformReady` builds on it, so a window upgrades from config-only to full without a second IndexedDB connection; returns `{ configManager, attachMode, platformClient }`. Where SharedWorker is unavailable the window falls back to bootstrapping itself (attach when a prior window seeded, else seed) — `platformClient: null`
- two SharedWorker connections per window (worker-split) — `ensureDataServicesHub` owns a per-`appId` `HubConnection`: the platform-services worker (`mkt-platform-services:<appId>`, spawned FIRST, `platformClient` — catalog RPCs + AppData) and the data worker (`mkt-data-services:<appId>`, `client` — providers, CSRM deltas, SSRM RPCs); `warmHubConnection()` opens both early (never throws) so the workers spawn while ConfigManager init runs, and the hub bundle adopts the same ports (`bootstrapDataServices({ client, appDataClient })`); the bundle exposes both as `client` / `platformClient`, and `DataServices.platformClient` carries the second one to React; `warmPlatformConnection()` / `PlatformConnection` spawn or reuse the platform-services port alone (config-only windows never spawn the data worker), and `getOrCreateHubConnection` composes the data worker on top of that same port
- `sendWorkerBootstrap(port, input)` + `worker-bootstrap` protocol message — main thread posts the deployment bootstrap (`appId`, `userId`, seed URL, REST URL) on the worker port immediately after `new SharedWorker()`; sent by `createDataServicesWorker` and `createPlatformServicesWorker` (one handshake per worker). The port is the only channel that reaches a SharedWorker: the previous localStorage transport could never work (a worker global has no Storage API, so every field arrived `undefined` and `configServiceRestUrl` was silently ignored). A blank `userId` is an explicit "boot anonymous" answer; if no handshake arrives within 5 s the worker warns and boots local/anonymous
- `AdoptedPort` / `installSharedWorkerHub({ adoptPorts })` — port handover for entries that must accept connections before the hub exists. `defaultEntry` cannot build its ConfigManager until the handshake arrives, so it buffers each port's messages and replays them through the hub, in arrival order, on adoption — no early `appdata-attach` is lost; the shared installer attaches EVERY port (adopted or `onconnect`) before the hydrate awaits and backlogs dispatch, in arrival order, until the host is ready — a started port with no listener drops messages, and the previous attach-after-hydrate ordering lost every request a first window sent mid-hydrate (its AppData attach, `hub-ready`, the first `get-config`): WORKLOG item 14's forensic cause, pinned by `entry.adoptPorts.test.ts`
...
- `ConfigManagerOptions.writer` (`ConfigWriter`) — route `appConfig` writes (`saveConfig` / `deleteConfig`, and through them `createConfig` / `updateConfig` / the `profiles` namespace / `createConfigServiceStorage`) to a delegate instead of this context's IndexedDB / REST path; the delegate resolves with the row as persisted and the manager adopts those stamps into its row cache, so a later optimistic-lock check matches IndexedDB; the change notifier still fires locally. Windows pass the platform-services client; workers have none (worker-split W2 single writer)
- `SharedWorkerDataServicesHub` — the DATA-PLANE worker state machine (providers, cache, fan-out) — since the worker split (W1c) it serves **no config catalog and no AppData**: `hub-ready` / `get-config` / `list-configs` / `config-invalidate` / `appdata-*` are answered by `PlatformServicesHost` only and the data hub ignores them; attach with matching `extra` overlay (e.g. same historical `asOfDate`) late-joins without a second upstream `restart`; a create / restart / reconfigure attach is a **lifecycle moment**: the hub first runs `ProviderLifecycleReads.prepare()` (one-row provider cfg read for cfg-free attaches + a fresh AppData snapshot, both from shared IndexedDB), then creates / restarts — a `detach`, `port-close` or `dispose` that lands during the read cancels the attach (`pendingAttaches`), concurrent cfg-free attaches share one provider, a failed read answers `status: error`; hubs without a ConfigManager (tests) stay synchronous; **`buildIntrospectSnapshot()`** / `hub-introspect` RPC reports running providers + subscribers (no catalog — the inspector merges the platform host's answer via `mergeHubIntrospect`); **`provider-running`** RPC — O(1) scalar "is this provider running?" probe for the window-open path (never serializes hub state); all fan-out is inline from the hub thread (encode once, flat byte copy per port; the per-subId fan-out worker pool was removed 2026-07 — see `docs/hub-fanout-optimizations.md` §12); frames encode + broadcast only when the provider has data listeners (stats-only subscribers keep the cache warm without encode work); **`subscription-lost`** event + client auto re-attach when hub evicts stale subscribers; extended ping grace (`SUBSCRIBER_PING_TIMEOUT_HIDDEN_MS`) when client reports `meta.hidden`; subsystems live in sibling modules — `SubscriberRegistry` (listener membership, O(1) `subId` index for ping/detach/evict), `ProviderLifecycleReads`, `HubSsrmRpc` (SSRM block RPC dispatch, engine sessions, tick loop), `HubStatsSampler` (1 Hz diagnostics sampler), `providerEmit.ts` (upstream event application + encode), `replayCache.ts` (bucketed late-join replay: live ticks dirty only touched ≤500-row buckets, attach re-encodes dirty buckets and reuses clean buffers), `hubIntrospect.ts`, `hubStats.ts`; the file holds at the 800-line ceiling
- `PlatformServicesHost` — the platform-services worker's brain (worker-split W1b/W1c): config catalog RPCs (`hubCatalogRpc.ts` handlers over `ConfigCatalogCache`) + `HubAppDataService` (the sole AppData IndexedDB writer) and NOTHING of the data plane — its event loop never carries ingest, so a tool window's `list-configs` / AppData attach never queues behind a streaming blotter; installed by the same bundled asset via `installPlatformServicesHost` (`entry.ts` shares one port-lifecycle `install()` with the data hub; `onconnect` is registered before hydrate so early ports queue); `hub-introspect` answers honestly (zero providers, catalog idle rows, AppData), `provider-running` is always false; data-plane requests are ignored, never faked
- `ProviderLifecycleReads` — the data hub's ONLY config/AppData coupling after the split: a READ-ONLY ConfigManager wrapper with `prepare(providerId | null)` (parallel one-row `DataProviderConfigStore.get` + `AppDataConfigStore.list`), `resolveProviderConfig`, `refreshAppData` (single-flight), a synchronous `lookup` for the transports' `appDataLookup` contract and `snapshotRows()` for tracing / introspect; correct by construction — every lifecycle moment re-reads shared IndexedDB, no invalidation channel and no worker↔worker bridge
- `HubSsrmRpc` — the SSRM slice of the hub (`ssrm-*` RPC dispatch + `ssrm-rpc` replies, `attachSession` / `detachSession`, `bootProvider` / `dropProvider` on the WASM plane, `warmSessions`, the tick loop `flushTicks` with per-subscriber `viewDelta` routing); `HubStatsSampler` — arm / stop / `flush` / `emitStopped` + the 1 Hz bucket rotation; both lent hub state through a small context and extracted verbatim
- `ReplayScheduler` — round-robin late-join replay fan-out (worker-split W4): every port replaying a provider at once is a pending job; a pass runs ROUNDS (one chunk to every pending port per round, visible windows before hidden — the only backpressure a MessagePort exposes), checks `REPLAY_PASS_BUDGET_MS` (8 ms) between rounds, then yields to the macrotask queue (`yieldToMacrotask`, a MessageChannel hop — no timer clamping) so ingest and block RPCs breathe; a lone window whose replay fits the budget still ships in one synchronous pass, unchanged; each job's chunk set is FROZEN at enqueue (`ensureReplayChunks` at attach — dirty buckets re-encoded once per attach burst, clean buffers shared across the burst; re-resolving per pass under a sweeping feed re-encoded most of a 20k-row cache 30+ times and tripled hub-thread time); live deltas for a port whose replay is in flight are DEFERRED and flushed after its `ready`, so no snapshot interleaves a live frame; detach / port-close / stop / recreate cancel; accounting (`passes`, `replays`, `chunksPosted`, `encodeMs`, `hubThreadMs`, `lastEpisode { ports, chunksPosted, encodeMs, hubThreadMs, wallMs }`) rides the data hub's `hub-introspect` as `fanout` (`HubFanoutIntrospect`) for plan §5's hub-thread row. `refresh-provider` and attach replays share it; an attach replay of an empty cache still bypasses it. SharedArrayBuffer fan-out (the plan's gated stretch) is NOT implemented — it needs `crossOriginIsolated`, which the web previews do not have
- `hub-introspect.ssrm` (`HubSsrmIntrospect`, data hub only) — where the data worker's thread goes when N grids share one SSRM provider: per block read (`ssrm-get-rows`) the **queue wait** (from the client's `sentAt` epoch stamp on every SSRM RPC — `SsrmRpcTiming`) and the **engine time**, the same pair for every other `ssrm-*` RPC, per tick flush the cost + the session count + ticks posted + upsert rows posted / withheld by trimming, and per ingest batch the engine time; each as count / total ms / p50 / p99 / max over the last 256 samples (`LatencyReservoir`). `ssrm-multiwindow.mjs` prints it as `[hubSsrm]` at the end of a run
- `SsrmSessionWindows` (data hub) — per-session SSRM tick trimming. The engine's shared row delta is the WHOLE table's churn (every upserted row, full width) and the hub used to post that identical payload to every session of the provider; now the hub remembers the leaf keys of every flat block a session reads (`ssrm-get-rows` → `composeRowId` with the provider's `keyColumn`) and posts each `rowDelta` tick to that session with only the rows it holds plus `SsrmTickPayload.unloaded: { upserts, removals }` counts for the rest (a tick that touches nothing loaded still carries the counts; one that touches nothing at all is not posted). Never guesses: a session that has not read a flat block, a grouped session (group rows have no leaf keys) and a session past 50 000 keys (`DEFAULT_MAX_SESSION_KEYS`) get the full delta; detach forgets the window. Measured on the live twelve-view OpenFin platform: one view received 4.5 MB/s of ticks (31 per 10 s, up to 3.2 MB each, 38 000 full rows) for a grid holding two blocks — after trimming 0.10 MB/s (62 ticks per 10 s, ≤31 kB each, 794 rows), the worker withholding 95 % of the upserts it used to broadcast (`upsertsWithheld` 12.0 M vs `upsertsPosted` 0.67 M over the run); a fling's blank rows fill in 1.8 s instead of 3.8–4.3 s and a cold view issues its first block 12.6 s after reload instead of 20–28 s
- `bindSsrmTicks` honours `SsrmTickPayload.unloaded`: withheld upserts and removals drive the count check / positional refresh exactly like an upsert with no loaded node, so a trimmed tick changes nothing about when the grid re-counts or re-reads
- `useDataProviderConfig` reports `{ cfg: null, loading: true }` on the very render a new `providerId` appears (each stored view is stamped with the id it describes and the returned view is derived synchronously) — no one-render window of the previous id's `{ cfg: null, loading: false }`; `MarketsGridContainer` also treats a chosen provider with no cfg and no error as loading, so it never mounts its `__no_provider__` placeholder grid ahead of the real one (WORKLOG 20: every blotter used to build its AG Grid twice)
- `MarketsGridSsrmConfig.maxConcurrentDatasourceRequests` and `blockLoadDebounceMillis` — opt-in pass-throughs to AG Grid (defaults untouched: 2 in flight, no debounce), also on `MarketsGridProps.ssrm`. Measured on a twelve-view OpenFin platform: four in flight did not shorten a fling's fill (4.1 s vs 3.8 s) because the view's own rendering, not the worker round trips, is the wait — so neither is defaulted on
- `mergeHubIntrospect(data, platform)` — one operator view from the two workers' `hub-introspect` answers: providers + ports from the data hub (running rows named from the catalog), catalog readiness / count + AppData from the platform host, idle catalog rows appended; exported from the package root for the inspector drawer
- `ConfigCatalogCache` — worker-side in-memory data-provider catalog (`loadAll`, `get`, `getProviderConfig`, `list`, `invalidate`, `upsert`); `ensure(providerId)` resolves one provider on demand (cached row, else a single `ConfigManager` read with no full `loadAll`) and caches it; owned by `PlatformServicesHost` (the data hub keeps no catalog since W1c)
...
- `ISsrmDataProvider` — SSRM client contract at `IDataProvider` lifecycle parity (`capabilities`, `start` / `stop` / `refresh` / `restart`, `getRows`, `getColumnValues`, `getRowCount`, `getAggregates`, `watchGroups`, `onSsrmTick`, `onRefresh`); no `getData()` / `onSnapshotData()` / `onTick()` — rows stay in the worker cache. `getAggregates` feeds the status bar and expression `SUM` / `AVG` / `MIN` / `MAX` / `COUNT`. Optional `applyEdits({ rows, editedColumns? })` writes grid edits (whole rows) into the engine cache — every grid on the provider sees them and refreshes keep them. The upstream feed is not written to; naming `editedColumns` makes the worker hold each edited cell as an overlay over whole-row upstream resends until the upstream value itself changes (a new value or an echo of the edit releases the hold — also released on row removal, config change, and beyond a 10 000-row FIFO cap); without it the row's next upstream tick reverts the edit. Optional `status` exposes the last hub status so the datasource can hold its first block until the snapshot is in
- `SsrmProviderClientAdapter` — hub attach in `ssrm` mode (no CSRM cache replay) + `ssrm-get-rows` / `ssrm-column-values` / `ssrm-row-count` / `ssrm-aggregates` / `ssrm-watch-groups` / `ssrm-apply-edits` RPC + `ssrm-tick`; tracks `status` from the hub's status events (`loading` until told otherwise, reset on detach). `refresh()` emits `onRefresh` so bound grids purge and re-read blocks (the SSRM stand-in for CSRM cache replay); `restart()` purges too, since the reconnect re-boots the worker cache
- `IDataProviderFactory` — `getProvider(providerId)` factory surface
- `ProviderClientAdapter` — client-side `IDataProvider`; cfg-free subscribe, `SnapshotReassembler` snapshot assembly, `getProvider()` on hub bundle. `getData()` returns the last snapshot commit by reference (not copied, not updated on live ticks). `onReset` deliveries update snapshot subscribers (mid-stream STOMP reconnect). `start()` resolves its one provider via the worker's on-demand `get-config` (single-row read, no full-catalog gate) so attach is race-safe even mid-preload Takes `catalogClient` (defaults to `client`) so the `get-config` read rides the platform-services worker while attach / subscribe stay on the data client; `SsrmProviderClientAdapter` takes the same option; `ensureDataServicesHub().getProvider()` and the React `useDataProvider` / `useSsrmDataProvider` hooks pass the bundle's `platformClient`
...
- **Mock SSRM** (`providerType: 'mock-ssrm'`, `MockSsrmProviderConfig`) — the worker's rich mock generator (positions/trades) ingested into the SSRM WASM engine instead of the CSRM cache: the broker-free parity twin of `stomp-ssrm` (same generator the CSRM mock provider streams, so a CSRM and an SSRM grid on one dataType render the same book). Routing branches on `isSsrmProviderType` (types package: `stomp-ssrm` | `mock-ssrm`; the hub, emit router, container auto-pick and capabilities all classify rather than compare literals), and the plane boots from the shared `SsrmProviderConfig` union. `RustHubHost.ensure` memoises the in-flight engine creation — `boot()` and a first `ingest()` racing in one tick used to build two engines under an injected factory, landing the snapshot on the losing one
...
- `toViewSpec` / `toViewSpecResult` / `ssrmEpochOf` / `ssrmEpochColumn` — AG Grid request → engine view spec. Two engine facts verified against the vendored WASM directly: the sort direction key is `sort` (a `dir` key is ignored and the view comes back ascending — every descending sort was silently ascending before this), and string columns answer `equals` / `contains` / `blank` but never order. The quick filter splits on whitespace and ANDs one OR-of-`contains` node per word across the search columns (AG Grid's own semantics: every word must match some column). Date columns get a numeric epoch shadow (`<col>__epoch`, `f64`) declared at boot and stamped at ingest by `SsrmWasmPlane`; date filters are DAY comparisons rewritten onto it as numeric ranges (`equals` → the UTC day's `inRange`, `notEqual` → `or` of before / after, `greaterThan` → after the day's end, `inRange` spans whole days; `blank` / `notBlank` stay on the string), and a sort on a date column sorts the shadow. A date condition with no bound is reported as unsupported; a column not declared as a date keeps the raw AG Grid bound. Pivot mode translates to `splitBy` + `columns` only when at least one row group is present — probed: the engine pivots grouped views only, `splitBy` alone returns flat leaves — so a pivot with no row groups is reported as unsupported instead of rendering silently un-pivoted
...
- `SsrmWasmPlane` — per-`providerId` façade: boot schema from `columnDefinitions` + `keyColumn` + `searchColumns`, ingest STOMP emits, translate starui RPCs to wasm control. `resolveSearchColumns` — an explicit `searchColumns` wins, otherwise every non-numeric, non-boolean column is searched (what the search bar promises); `dateColumnsOf` declares an `f64` epoch shadow per date column in the boot schema and `ingest` stamps it (`Date.parse`, null when unparseable) so date ranges and sorts can run on a column the engine can order. `ssrm-apply-edits` (`applyEdits`) ingests grid edits through the same path as an upstream message and records an **edit overlay** per named edited cell: a whole-row upstream resend of the pre-edit value is rewritten so the edit survives (the `legacy` wire shape resends full rows and `apply_message_json` upserts whole rows), while an upstream value that genuinely moves — or echoes the edit back — releases the hold; overlays die with row removals, config changes (boot-signature diff; a same-config re-boot from a second attach keeps them), and beyond `MAX_EDIT_OVERLAYS_PER_PROVIDER` (10 000 rows, FIFO). Retention is engine-side since T2: `apply_message_json` pins the table on ingest (`ensure_pinned`), so rows land whether or not any session has subscribed and survive every viewer leaving; `dropTable` (provider stop) releases the pin — the old hub-owned anchor-subscription workaround is deleted. The restart flush (`replace: true`) routes through `replace_snapshot` — atomic truncate + ingest in one revision, so a snapshot that SHRANK shows exactly the new rows, stale keys reach grids as delta removals, and a surviving key is never emitted as a removal (the engine drops a deletion superseded by a live re-upsert). `deleteRows` deletes by key. `warmRootView` opens and 1-row-reads the root view (no filter / sort) for a session off the request path; the hub calls it when the snapshot lands (`ready`, via `ProviderEmitContext.warmSsrm`) and for late-joining SSRM attaches, so the first block read hits an already-built view (second-window cold: ~250 ms measured). `getRows` under `splitBy` derives `pivotResultFields` from the window's `key|…|valueCol` keys (`SSRM_PIVOT_FIELD_SEPARATOR`, probed `|`; the engine reports no field list of its own), sorted for a stable secondary column tree. `getRows` returns `unsupportedFilters` alongside the rows when `toViewSpec` had no translation for a condition (the worker console warning alone was invisible to users). `pollAllTicks` drains the engine's `tick()` ONCE per flush and routes each session's group deltas to the provider that session subscribed to (tracked at `subscribe`), then polls `poll_shared_delta` per booted provider — the hub fans out from that map, so two `stomp-ssrm` providers no longer receive each other's deltas; `pollTicks(providerId)` remains as a single-provider convenience. `getColumnValues` answers set-filter lists as a one-level grouped view (the engine already returns one row per distinct key, so no new WASM entry point), excluding the column's own filter so de-selecting a value doesn't remove it from the list while honouring the quick filter like any other condition. `getAggregates` answers the status bar and expression aggregates (`sum` / `avg` / `min` / `max` / `count` over the current filter) via the engine's `aggregates` control. `getRowCount` answers saved-filter pill badges by reading a one-row window — the engine reports the view's total whatever window was asked for, so the count costs no row materialisation. Views are cached by canonical query signature (filter order normalised, since the array is ANDed) and capped per session in two partitions — `block` views (getRows: the level being scrolled, expanded groups) at `MAX_BLOCK_VIEWS_PER_SESSION` (24) and `poll` views (counts, value lists) at `MAX_POLL_VIEWS_PER_SESSION` (12) — so a poller can never evict a block view mid-scroll and rebuild a whole-dataset view on the block path; a block read promotes a shared poll-opened view (the root count view IS the root block view). Evicted views are disposed via the engine's `disposeView`: an engine view is live and maintained every tick, so opening one per read compounds until the worker stops answering and the grid hangs on "loading". Views are also dropped on session detach and on datasource re-boot, when the ids they hold are stale. The former no-delete engine limit is closed by T2 (`delete_rows` / `truncate` / `replace_snapshot` + pinned retention)
- **STOMP** (`startStomp()`)
  ...
  - Live phase → keyed deltas applied by `applyProviderToGrid` + `renderedRowUpdates` (widgets-react container): adds ride `applyTransactionAsync`; updates refresh in place — the node's data is already current (thin deltas patch the mirrored object; full rows are synced onto it with replace semantics), rendered rows get one `refreshCells({ rowNodes })` per `asyncTransactionWaitMillis` window with no `force` (AG Grid refreshes/flashes only changed cells), every changed node goes to `RowChangeBus.noteRowsChanged`; an active external filter makes every updated row a transaction unless its installer declared its columns on `platform.externalFilters` (`getExternalFilterColumns` seam, plan B2), in which case only those columns are position keys, and only rows whose sort / filter / group / aggregated-value column changed value (per-node key snapshot via `api.getCellValue`; first touch after a key-set change rides once) still ride an update transaction so AG Grid re-sorts / re-filters / re-aggregates along the changed path with rendered rows and edits kept. Quick filter, pivot, advanced filter or an external filter active → every updated row is a transaction, as before. No `rowIdField` → plain update transaction (the grid cannot address nodes). Refactor plan B1, WORKLOG 21
...
- Pre-encoded replay (`delta-bin`): cache replay chunks are UTF-8 JSON `Uint8Array`s built **once per cache generation** (lazy, invalidated O(1) on any cache mutation) and the same buffers are posted to every attaching port — N simultaneous window attaches cost one serialization plus N flat byte copies instead of N object-graph structured clones; client decodes back into the normal `onDelta` path Since W4 the chunks are posted by `ReplayScheduler` (round-robin across simultaneous joiners), not in one synchronous loop per port.
...
- Thin field-level deltas (`cfg.thinDeltas`, default off, requires `keyColumn`): post-ready live frames broadcast as `delta-patch` events carrying only the **changed top-level fields** per row (`RowPatch { k, s?, d?, f? }`, `diffTopLevel` in `wire/rowDiff.ts`) — touch updates that change a few fields of a wide row shrink the hub→window wire by the touch ratio; inserts / non-diffable rows ship full under `f`, observably-unchanged rows are skipped entirely (free conflation); the hub cache keeps full rows so replace frames + late-join replay are unaffected; patch batches ≥ `LIVE_BIN_MIN_ROWS` encode to UTF-8 JSON once and byte-copy per port; the client mirrors full rows per thin subscription (`sub-init` handshake carries `keyColumn`) and patches each row **in place** (`Object.assign` of the changed fields onto the mirrored row, which is the very object the consumer and AG Grid's row node already hold) and delivers that same object — no per-patch full-row copy (a 372-column row for a 4-field patch at ~5 000 rows/s per view was half of a docked window's main-thread time, WORKLOG 21); downstream change detection is by value (cells vs last-rendered; the row-change bus hears the changed nodes from AG Grid's flush event or from the rendered-row apply path's `noteRowsChanged`), so a consumer that needs a row's previous values must copy them before the patch lands
...
- Client→worker config writes (W2 single writer): `config-save { row, expectedUpdatedTime? }` / `config-delete { configId }` — served by `PlatformServicesHost` only; `SharedWorkerDataServicesClient.saveConfigRow()` / `deleteConfigRow()` are the client surface, and a window's `ConfigManager` reaches them through its `writer` option
- Worker→client catalog events: `catalog-ready`, `config-snapshot` (responses for hub-ready/get/list/invalidate/hub-introspect/provider-running/config-save/config-delete; a stale `config-save` answers `ok: false` with `code: 'optimistic-lock'` + `conflictRow`, which the client rethrows as `OptimisticLockError`)
...
- `HubAppDataService.resync()` (platform-services worker) — reload AppData provider rows from IndexedDB on throttled mirror re-attach and after catalog `config-invalidate` (AppData editor saves)
...
- `ensureConfigReady()` — thin-window config tier: platform-services port + read-only IndexedDB + catalog gate (singleton per `appId`); see the runtime-architecture bullet
- `ensurePlatformReady()` — spawns both workers (platform-services first), rides the `ensureConfigReady` tier, then connects the data hub (singleton per `appId`; reuses `ensureConfigReady`'s ConfigManager). Returns once config + hub connection are established; full hydration (`bundle.ready`), `markPlatformReady`/`markPlatformWarm`, and AppData-bootstrap hooks (off `bundle.appDataReady`) all settle in the background so the window paints without waiting on catalog/AppData
- `acquireBackgroundFreezeExemption()` (called by `ensurePlatformReady`) — every data window holds a Web Lock named `starui-background-freeze-exemption` so Chromium's page-freezing policy exempts it while hidden / minimized; retries until actually granted (a boot-time request can reject while the document is not yet fully active); requested in **shared** mode — an exclusive request under the origin-wide name was granted to the first window only while every later window queued forever (seen live on the star-demo OpenFin platform: provider window held, twelve views pending), and a queued request neither rejects nor retries
- `warmPlatform(config, { workerScriptUrl?, providers?: 'autoStart' | string[] })` — the one-line app-load / OpenFin-dock warm-up (W2): `ensurePlatformReady` plus provider warm-up under a deliberate name, merged with the lazy grid-mount path through the same per-`appId` promise maps (first caller owns the flight, nothing double-boots, repeat calls no-op per provider); `'autoStart'` starts every catalog row whose transport cfg has `autoStart: true`; warm-up is a STATS-mode attach on the data client, so the provider runs (cfg from the hub's lifecycle read) but nothing is encoded or fanned out until a data listener appears, and closing the warming window lets idle teardown reclaim it; never rejects (a failed warm-up is logged, the lazy path still boots everything); traces the whole start-up to the console under a single **`[provider-startup]`** tag — how many catalog rows carry `autoStart` out of how many exist, each chosen id with its `providerType`, how many were started vs. already warm, and then per provider (via the stats the warm-up attach used to discard) the hub running it, the snapshot landing (`N rows in Xms`) and each new error, one line each. In OpenFin these land in the session-long **provider window's** console (`#/platform/provider`), which is where `warmPlatformFromProvider()` runs — not the dock's own window
- `ProviderWarmupConfig.autoStart` (`@wellsfargo-starui/types/shared`) — the warm-up flag typed and documented on EVERY `ProviderConfig` member (it was an undocumented STOMP-only field that `warmPlatform` read through a cast); set from the provider editor's Behaviour tab
...
- `createDataServicesWorker()` / `createPlatformServicesWorker()` — the two SharedWorker constructors behind `ensurePlatformReady` (the single-worker `createDataServicesClient` / `bootstrapDataServicesWithWorkerAsset` helpers were deleted with the worker split: they spawned only the data worker and attached AppData on it, which no longer answers)
...
- `DataServicesProvider` — context exposes `client` (data plane) and `platformClient` (catalog + AppData; `services.platformClient ?? services.client`, so single-worker bundles keep working); `configStore` calls `platformClient.invalidateConfig()` after editor `save`/`remove`
...
- `useDataProviderConfig(providerId)` — single provider row from the platform-services worker's catalog cache (`getProviderConfig` RPC on `platformClient`; `useDataProvidersList` likewise); stale-while-revalidate on scoped `catalog-ready` (same `providerId` or `full` only); switching `providerId` drops the previous provider's cfg immediately (`cfg: null, loading: true`) instead of exposing it during the new fetch; each fetch is time-bounded (2.5s × 3 attempts, silent re-issue on no-response only — a worker boot-window loss becomes a sub-3s blip instead of a permanent spinner; explicit rejections surface immediately, keeping the stale cfg)
...
- `HubInspectorDrawer` / `HubInspectorHost` — shadcn drawer listing running + idle catalog providers (display name + id, status, subscribers, cache row counts, expandable worker-loaded `cfg` JSON) and AppData rows (expandable `values`); polls `getHubIntrospect()` while open Since the worker split it polls BOTH workers (`client` + `platformClient`) and renders `mergeHubIntrospect` of the two answers.
...
- `enableInspectContextMenu(opts)` / `enableInspectContextMenuInLayout(layout)` (internal) — keep right-click → **Inspect** on every platform view and window. `@openfin/core` marks both `contextMenu` (`@default true`) and `contextMenuSettings` **deprecated, superseded by `contextMenuOptions`**, and `@openfin/workspace-platform`'s Browser window normalizer writes `contextMenuOptions: { template: [], enabled: false }` onto anything not already carrying a NON-EMPTY template (a supplied one survives, minus `print`) — so a platform window has no context menu at all unless a template is supplied, and the deprecated pair cannot re-enable it. Installs `['cut','copy','paste','selectAll','separator','reload','inspect']`, keeps a caller's own non-empty template and only tops up `inspect`/`reload`, and replaces the empty-template off switch. Applied in the `createView` / `createWindow` overrides beside `disableBackgroundThrottling` and for the same reason — OpenFin persists fully-resolved options, so every layout saved while the menu was off would otherwise restore it forever; the manifest's `defaultViewOptions` / `defaultWindowOptions` carry the same template
