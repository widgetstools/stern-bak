...
Expected: `setup:tarball`'s cleanup step removes the stale `vendor/wellsfargo-starui-host-openfin.tgz` and `vendor/wellsfargo-starui-openfin-platform.tgz` automatically, and `build:tarball` builds all six tarball-track apps clean. If it fails specifically because a generated `tarball/*/package.json` in the **apps repo** still declares a dependency on either retired name, that is a real, expected finding (sub-phase 1 hit the equivalent case for `icons-svg`) — do not edit files in `/Users/develop/wfh/starui-apps` as part of this task; note it for Task 2's WORKLOG update instead.
