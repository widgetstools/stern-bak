...
| Client contract | `packages/data/host-data/src/provider/ISsrmDataProvider.ts`, `SsrmProviderClientAdapter.ts` | `getRows / getColumnValues / getRowCount / getAggregates / watchGroups / watchPredicate? / unwatchPredicate? / applyEdits? / status?` + `onSsrmTick / onRefresh / onStatus` |
| Wire | `packages/data/host-data/src/runtime/protocol.ts`, `runtime/client/SharedWorkerDataServicesClient.ts` | `ssrm-get-rows / -column-values / -row-count / -aggregates / -watch-groups / -watch-predicate / -unwatch-predicate / -apply-edits` RPCs (`ssrmRpc`, 15 s timeout), `ssrm-tick` push (`rowDelta` / `groupDelta` / `viewDelta`) |
...
| Plane | `runtime/ssrm/SsrmWasmPlane.ts`, `toViewSpec.ts`, `ssrmTypes.ts` | Per-provider façade over the vendored Rust hub; AG Grid request → engine view spec (computed columns ride the request since T3); view cache (24 / session); predicate watches off the LRU; typed date columns since T6 |
...
## 2c. What changed on 2026-09-12 (engine pass — plan §12 T3–T7 landed)

One rangrez commit (`feat(hub-rust): table lifecycle + computed views, aggregates,
deltas, dates, pivot`) plus this repo's integration. All five remaining engine phases:

- **T3 — computed columns.** `src/expr.rs` evaluates the T1 wire grammar with the client
  evaluator's JS coercions verbatim; conformance corpus is GENERATED by this repo
  (`ssrmExpressionContract.wire.json`, 44 compiled cases) and copied into rangrez —
  regenerate + re-copy whenever the fixtures grow. Views take `computed: [{as, expr}]`;
  values memoize per revision and patch via the cache touch log; filter/sort/groupBy/
  aggregates/splitBy resolve computed names; rows carry the values. Client:
  `compileSsrmComputedColumns` (grid) compiles tier-`compiled` calculated columns into
  every request via `createSsrmDatasource.getComputedColumns`; `lockSsrmExpressionColumns`
  now takes the engine-backed set and leaves those columns live.
- **T4 — aggregates.** Engine `Agg` + expression agg scalars gained median / stdev /
  variance / distinct_count; expression scalars resolve over the view's FILTERED rows
  with CLIENT fold semantics (`expr::client_aggregate`) and converge one revision behind
  a scalar-dependent filter (the client binder's own temporal behaviour). Classifier
  promotes the statistical aggregate forms to `compiled`; the `unsupported` tier and
  `loadedRowAggregates` are deleted.
- **T5 — membership deltas.** `watch: true` views report entered/left keys per revision;
  wasm `tick()` emits `viewDelta` to the opening session. Plane `watchPredicate` /
  `unwatchPredicate` (predicate = computed bool column + `equals true` filter, held OFF
  the view LRU), wire RPCs, provider methods, and the alerts runtime's
  `bindSsrmAlertPredicates`: compiled dataChange rules fire over the whole book through
  the same dispatcher; engine-watched rules are excluded from client evaluation; the
  worker fans a `viewDelta` only to its `watchSubId`.
- **T6 — typed dates.** Boot schema types date columns `date`; `TableCache` parses the
  epoch at write into a side vector (hand-rolled ISO-8601 — the 1.78 toolchain pin rules
  out chrono); `query_value` serves instants to sort/filter, `row_json` keeps the string.
  The client `__epoch` machinery (`SSRM_EPOCH_SUFFIX`, `ssrmEpochColumn`, `stampEpochs`,
  `ssrmEpochOf`, the `dateNodes`/sort rewrites) is DELETED.
- **T7 — pivot.** Group-less `splitBy` = one grand-total row; split values fold `|`→`¦`.
  The `toViewSpec` guard is gone, and the group-less request now populates `aggregates`
  from `valueCols` (found by the new integration pin — the old path only built them for
  grouped requests).
- **Found along the way:** `resultOf` in the plane swallowed engine `{type:'error'}`
  replies (surfaced as "openView returned no viewId" instead of the engine's message —
  e.g. WHICH computed column failed to parse). Fixed.
- `capabilities()` gates it all client-side; six new pins in
  `SsrmWasmPlane.wasm.integration.test.ts` (12 total). Parity matrix: 14 full /
  2 partial / 0 gap (Calculated and Alerts went full).
...
| String columns: `equals`, `equalsIgnoreCase`, `notEqual`, `contains`, `startsWith`, `in`, `blank`, `or` work. `greaterThan`, `lessThan`, `greaterThanOrEqual`, `inRange` on strings return **nothing**. | AG Grid's text filter has no ordering ops, so text is fine; date columns order via their typed epoch (T6, below). |
...
| ~~Boot schema types: `f64`/`bool`/`string` only; no date type, hence `__epoch`~~ **Fixed engine-side (T6, 2026-09-12):** `date`/`dateString`/`timestamp` types parse the epoch at WRITE time in `TableCache`; sort/filter compare the instant, `row_json` keeps the display string. | The client `__epoch` shadow machinery is DELETED; date filters send numeric day bounds against the REAL column. |
| Every boundary crossing is a JSON string (control in, rows out) and rows are then structured-cloned to each window. | Double serialisation per block (§5, item 1). |
...
| ~~Rows applied while the datasource has ZERO subscribed sessions are dropped~~ **Fixed engine-side (T2, 2026-09-11):** ingest pins the table (`ensure_pinned`), so retention follows the data, not the viewers. | The plane's anchor-subscription workaround is deleted; `drop_table` (provider stop) releases the pin. |
| ~~`splitBy` pivots grouped views only; `\|` unescaped~~ **Fixed engine-side (T7, 2026-09-12):** group-less `splitBy` serves the one grand-total row, and split VALUES fold `\|` → `¦` before joining. Pivot fields stay `<key>\|…\|<valueCol>` and the payload still carries no field list. | The plane derives `pivotResultFields` from the window; the surface sets `serverSidePivotResultFieldSeparator: '\|'`; the pivot-needs-row-groups guard is gone. |
| ~~No delete primitive~~ **Fixed engine-side (T2, 2026-09-11):** `delete_rows` / `truncate` / `replace_snapshot` (atomic truncate+ingest, one revision), plus the superseded-removal rule — a key deleted then re-upserted in one delta window rides the upserts, never the removals. (Per-row op-tagged deletes had always existed behind the unconfigured `opField`.) | A restart that shrinks the dataset now shows exactly the new rows; the restart flush routes through `replace_snapshot` instead of the re-boot that never dropped anything. |
...
| **T3 (2026-09-12):** view specs take `computed: [{as, expr}]` (wire grammar v1) — evaluated engine-side with the CLIENT's coercion semantics (44-case generated corpus, `tests/expr_contract.rs` in rangrez), first-class in filter/sort/groupBy/aggregates and on every row. A computed entry that fails to parse REJECTS `openView` with the offender named. | `compileSsrmComputedColumns` rides tier-`compiled` calculated columns on every request; `lockSsrmExpressionColumns` skips them; expression agg scalars (full T4 set incl. median/stdev/variance/distinct_count) resolve over the view's filtered rows. |
| **T5 (2026-09-12):** `watch: true` views diff their filtered membership per revision; `tick()` pushes `viewDelta {viewId, entered, left, rows≤200}` to the opening session. First refresh primes silently. | `watchPredicate` (plane → worker RPC → provider) + the alerts runtime's engine predicate bridge: compiled dataChange rules fire book-wide; deltas are pinned to the registering subId so two windows never double-fire. |
| `capabilities()` on the wasm hub names the feature set (`computedColumns`, `viewDeltas`, `dateColumns`, `pivotWithoutGroups`, aggregate list). | The plane gates on it: an engine build without a capability REFUSES the feature loudly (unsupported-filter report / thrown watch) instead of silently mis-serving. |
...
| Measurement | Value |
|---|---|
| Cold to first rows, window 1 (fresh worker, snapshot stream) | 5.9–7.0 s (broker pacing dominates); **4.7 s after T2** (engine-side retention — no anchor round-trips on the ingest path) |
...
**1. Double serialisation on every block.** Rows are JSON-stringified inside the WASM boundary,
...
**2. Per-level SSRM store options are unset and unmeasured.** `getServerSideGroupLevelParams`,
...
**3. Scale the multi-window soak.** `ssrm-multiwindow.mjs` ran at PAGES=2 (numbers in §4b). The
six-blotter shape from the CSRM diagnosis is `PAGES=6 APP_URL="http://localhost:5215/?rate=10000"`
— not yet run.

**4. Line endings.** Several files carry LF in a CRLF working tree (git normalises on commit).
