...
2. **OpenFin — verified live (see plan §5 "the deployment shape, measured live" and WORKLOG 18).** With star-demo running in OpenFin 43.142 and twelve SSRM views streaming: platform-port config RPCs 0.3 ms p50 while the data port's scalar probe waited 55 ms p50 / 190 ms p99 — the starvation the split isolates, seen directly; `config-save` through the single writer and `warmPlatform(config, { providers: 'autoStart' })` from the provider window both worked (the seeded `test.dp` row predated the `autoStart` flag — seeding is empty-only — and was flipped live through `config-save`). Two things came out of it: the freeze-exemption lock was exclusive under one name, so only the provider window held it and every view queued forever — fixed to shared mode, but windows loaded before the fix keep the old behaviour until the platform restarts (restart the provider, then check `navigator.locks.query().held` in a view); and the workers carried ~45 dead ports / AppData listeners from an earlier, unreproduced lifecycle — clean close and reload both release ports — proposed fix in WORKLOG 18 (platform-port heartbeat + host sweep).
