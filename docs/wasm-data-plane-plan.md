...
Decision (2026-08-30): grids stay on the **client-side row model by default**
(SSRM later). **SSRM delivered 2026-08-31** as an opt-in
(`MarketsGridContainer` `rowModel="serverSide"`): a per-window Perspective
replica table — the replicated topology §5b measured at 36%/core — fed by the
SAME provider delta stream CSRM windows receive (no new hub protocol), with
AG Grid's server-side row model served entirely from engine views
(`packages/react-grid/widgets-react/src/container/markets-grid-container/ssrm/`,
ported from the ssrm-grid reference implementation). Grouping, aggregates,
sort, filters (simple/set/multi/advanced), pivot, grand total, child counts,
stable row ids, and live updates (leaf patch + engine aggregate patch +
count-change refresh) all run against the replica; the provider sub-worker
stays the transport authority.
